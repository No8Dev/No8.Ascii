...
namespace Asciis.Terminal.Core.Trees;
...
internal class Branch<T> where T : class
{
    ...
    {
        if (tree.TreeBuilder == null) return;

        var children = tree.TreeBuilder.GetChildren(Model) ?? Enumerable.Empty<T>();
    ...
    {
        return
            GetLinePrefix(driver).Sum(x => x.ColumnWidth()) +
            GetExpandableSymbol(driver).ColumnWidth() + (tree.AspectGetter(Model) ?? "").Length;
    }
    ...
    public virtual void Draw(ConsoleDriver driver, ColorScheme colorScheme, int y, int availableWidth)
    {
        ...
        var isSelected = tree.IsSelected(Model) && tree.HasFocus;
        var lineColor = isSelected ? colorScheme.Focus : colorScheme.Normal;

        driver.SetAttribute(lineColor);

        // Everything on line before the expansion run and branch text
        Rune[] prefix = GetLinePrefix(driver).ToArray();
        var expansion = GetExpandableSymbol(driver);
        ...
        if (tree.Style.ColorExpandSymbol || tree.Style.InvertExpandSymbolColors)
        {
            ...
            if (tree.Style.InvertExpandSymbolColors) color = new CellAttributes(color.Background, color.Foreground);
        ...
        if (lineBody.ToRuneList().Sum(l => l.ColumnWidth()) > availableWidth)
        {
            // remaining space is zero and truncate the line
            lineBody = lineBody
                      .ToRuneList()
                      .TakeWhile(c => (availableWidth -= c.ColumnWidth()) >= 0)
                      .ToString();
            availableWidth = 0;
        }
        else
        ...
        if (availableWidth > 0) driver.AddStr(new string(' ', availableWidth));
    ...
    private IEnumerable<Rune> GetLinePrefix(ConsoleDriver driver)
    {
        ...
        {
            for (var i = 0; i < Depth; i++) yield return (Rune)' ';
    ...
    {
        var leafSymbol = tree.Style.ShowBranchLines ? driver.HLine : (Rune)' ';

        if (IsExpanded) return tree.Style.CollapseableSymbol ?? leafSymbol;

        if (CanExpand()) return tree.Style.ExpandableSymbol ?? leafSymbol;
    ...
    public bool CanExpand()
    {
        ...
        if (ChildBranches == null)
        {
            ...
            if (tree.TreeBuilder.SupportsCanExpand) return tree.TreeBuilder.CanExpand(Model);
    ...
    public void Expand()
    {
        if (ChildBranches == null) FetchChildren();

        if (ChildBranches.Any()) IsExpanded = true;
    }
    ...
    public void Refresh(bool startAtTop)
    {
        ...
        if (startAtTop) Parent?.Refresh(true);
        ...
        else
        {
            ...
            var newChildren = tree.TreeBuilder?.GetChildren(Model) ?? Enumerable.Empty<T>();
            ...
            foreach (var toRemove in ChildBranches.Keys.Except(newChildren).ToArray())
            {
                ...
                if (Equals(tree.SelectedObject, toRemove)) tree.SelectedObject = Model;
            }

            // New children need to be added
            foreach (var newChild in newChildren) // If we don't know about the child yet we need a new branch
                if (!ChildBranches.ContainsKey(newChild))
                    ChildBranches.Add(newChild, new Branch<T>(tree, this, newChild));
                else //we already have this object but update the reference anyway incase Equality match but the references are new
    ...
    internal void Rebuild()
    {
        ...
        if (ChildBranches != null)
        {
            if (IsExpanded) //if we are expanded we need to updatethe visible children
                foreach (var child in ChildBranches)
                    child.Value.Rebuild();
            else // we are not expanded so should forget about children because they may not exist anymore
                ChildBranches = null;
        }
    }
    ...
    private bool IsLast()
    {
        if (Parent == null) return this == tree.roots.Values.LastOrDefault();
    ...
    internal bool IsHitOnExpandableSymbol(ConsoleDriver driver, int x)
    {
        ...
        if (!IsExpanded && tree.Style.ExpandableSymbol is not null) return x == GetLinePrefix(driver).Count();

        // if we could theoretically collapse
        if (IsExpanded && tree.Style.CollapseableSymbol is not null) return x == GetLinePrefix(driver).Count();

...
public class ShortcutHelper
{
    ...
    public virtual Key Shortcut
    {
        ...
        set
        {
            if (shortcut != value && (PostShortcutValidation(value) || value == Key.Null)) shortcut = value;
        }
    }
    ...
    {
        var key = kb.Key;
        if (kb.IsAlt && (key & Key.AltMask) == 0) key |= Key.AltMask;
        if (kb.IsCtrl && (key & Key.CtrlMask) == 0) key |= Key.CtrlMask;
        if (kb.IsShift && (key & Key.ShiftMask) == 0) key |= Key.ShiftMask;
    ...
    public static string GetShortcutTag(Key shortcut, string delimiter = null)
    {
        ...
        var k = shortcut;
        if (delimiter == null) delimiter = MenuBar.ShortcutDelimiter;
        string tag = string.Empty;
        var sCut = GetKeyToString(k, out var knm).ToString();
        if (knm == Key.Unknown)
        {
            k &= ~Key.Unknown;
        ...
        if ((k & Key.CtrlMask) != 0) tag = "Ctrl";
        if ((k & Key.ShiftMask) != 0)
        {
            if (!tag.IsEmpty()) tag += delimiter;
            tag += "Shift";
        }
        ...
        string[] keys = new string(sCut).Split(",");
        for (var i = 0; i < keys.Length; i++)
        {
            var key = keys[i].Trim();
            if (key == Key.AltMask.ToString() || key == Key.ShiftMask.ToString() ||
                key == Key.CtrlMask.ToString()) continue;
            if (!tag.IsEmpty()) tag += delimiter;
    ...
    {
        var sCut = tag;
        if (sCut.IsEmpty()) return default;
        ...
        for (var i = 0; i < keys.Length; i++)
        {
            ...
            {
                int.TryParse(k.Substring(1).ToString(), out var n);
                for (var j = (uint)Key.F1; j <= (uint)Key.F12; j++)
                {
                    int.TryParse(((Key)j).ToString().Substring(1), out var f);
                    if (f == n) key |= (Key)j;
                }
            }
            else
    ...
    {
        if ((key & (Key.CtrlMask | Key.ShiftMask | Key.AltMask)) == 0 &&
            !CheckKeysFlagRange(key, Key.F1, Key.F12)) return false;
    ...
    public static bool FindAndOpenByShortcut(KeyEvent kb, View? view = null)
    {
        ...
        foreach (var v in view.Subviews)
        {
            if (v.Shortcut != Key.Null && v.Shortcut == (Key)key)
            {
                ...
                if (action != null)
                {
                    view.Application.MainLoop.AddIdle(
                        () =>
                        {
                            action();
                            return false;
            ...
            if (FindAndOpenByShortcut(kb, v)) return true;
        }

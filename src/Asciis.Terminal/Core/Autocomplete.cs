...
public class Autocomplete
{
    ...
    /// </summary>
    public ReadOnlyCollection<string> Suggestions { get; protected set; } = new(new string[0]);

    /// <summary>
    /// The full set of all strings that can be suggested.
    /// </summary>
    /// <returns></returns>
    public List<string> AllSuggestions { get; set; } = new();
    ...
    /// </summary>
    public ColorScheme ColorScheme { get; set; } = Colors.Menu;
    ...
    public void RenderOverlay(View view, Point renderAt)
    {
        ...
        for (var i = 0; i < toRender.Length; i++)
        {
            if (i == SelectedIdx - ScrollOffset)
                view.Application.Driver.SetAttribute(ColorScheme.Focus);
            else
                view.Application.Driver.SetAttribute(ColorScheme.Normal);

            view.Move(renderAt.X, renderAt.Y + i);

            var text = TextFormatter.ClipOrPad(toRender[i], width);

            view.Application.Driver.AddStr(text);
        }
    }
    ...
    public void EnsureSelectedIdxIsValid()
    {
        ...
        while (SelectedIdx >= ScrollOffset + MaxHeight) ScrollOffset++;
    }
    ...
    {
        if (IsWordChar((char)kb.Key)) Visible = true;

        if (!Visible || Suggestions.Count == 0) return false;
        ...
        if (kb.Key == SelectionKey && SelectedIdx >= 0 && SelectedIdx < Suggestions.Count)
        {
            ...
            {
                // delete the text
                for (var i = 0; i < typedSoFar.Length; i++) hostControl.DeleteTextBackwards();
    ...
    /// </summary>
    public void ClearSuggestions() { Suggestions = Enumerable.Empty<string>().ToList().AsReadOnly(); }
    ...
    public void GenerateSuggestions(TextView hostControl)
    {
        ...
        {
            Suggestions = AllSuggestions.Where(
                                             o =>
                                                 o.StartsWith(
                                                     currentWord,
    ...
    private string? IdxToWord(List<Rune> line, int idx)
    {
        ...
        if (idx == line.Count) // the cursor positioned at the very end of the line
            areMidWord = false;
        else // we are in the middle of a word if the cursor is over a letter/number
        ...
        while (idx-- > 0)
        {
            if (IsWordChar(line[idx]))
                sb.Insert(0, (char)line[idx].Value);
            else
                break;
        }
        return sb.ToString();
    ...
    public virtual bool IsWordChar(char ch) { return char.IsLetterOrDigit(ch); }
}

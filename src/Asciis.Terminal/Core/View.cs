...
/// </para>
/// <para>
///    To flag a region of the View's <see cref="Bounds"/> to be redrawn call <see cref="SetNeedsDisplay(Rectangle)"/>. To flag the entire view
...
/// </para>
/// </remarks>
public partial class View : ISupportInitializeNotification, IDisposable
{
    ...
    protected AsciiApplication? application;
    public AsciiApplication? Application 
    { 
        get
        {
            if (application == null)
                application = SuperView?.Application;
            return application;
        }
        set
        {
    ...
    protected virtual void ApplicationChanged()
    {
        if (border != null)
    ...
    private Direction focusDirection;
    ...
    public Key HotKey
    {
        get => textFormatter.HotKey;
        set => textFormatter.HotKey = value;
    }
    ...
    public Rune HotKeySpecifier
    {
        ...
        set => textFormatter.HotKeySpecifier = value;
    }
    ...
    public Key Shortcut
    {
        ...
        set
        {
            if (shortcutHelper.Shortcut != value &&
                (ShortcutHelper.PostShortcutValidation(value) || value == Key.Null))
    ...
    /// </summary>
    public ConsoleDriver? Driver => Application?.Driver;

    private static IList<View> empty = new List<View>(0).AsReadOnly();

    // This is null, and allocated on demand.
    private readonly List<View> subviews = new();
    ...
    // This is null, and allocated on demand.
    private readonly List<View> tabIndexes = new();
    ...
    public int TabIndex
    {
        ...
        set
        {
            ...
            tabIndex = value > SuperView.tabIndexes.Count - 1 ? SuperView.tabIndexes.Count - 1 :
                value < 0 ? 0 : value;
    ...
    private int GetTabIndex(int idx)
    {
        ...
        {
            if (v.tabIndex == -1 || v == this) continue;
            i++;
        }
        ...
        set
        {
            if (!addingView && IsInitialized && SuperView?.CanFocus == false && value)
                throw new InvalidOperationException(
                    "Cannot set CanFocus to true if the SuperView CanFocus is false!");
            if (canFocus != value)
            {
                canFocus = value;
                if (!value && tabIndex > -1) TabIndex = -1;
                if (value && SuperView?.CanFocus == false && addingView) SuperView.CanFocus = value;
                if (value && tabIndex == -1) TabIndex = SuperView != null ? SuperView.tabIndexes.IndexOf(this) : -1;
                TabStop = value;
            }
            ...
            if (subviews != null && IsInitialized)
                foreach (var view in subviews)
                    if (view.CanFocus != value)
                    {
                        ...
                        {
                            if (addingView) view.addingView = true;
    ...
    internal Rectangle NeedDisplay { get; private set; } = Rectangle.Empty;
    ...
    /// </summary>
    public bool IsCurrentWindowTop => Application.CurrentWindow == this;
    ...
    public Rectangle Bounds
    {
        ...
        set => Frame = new Rectangle(frame.Location, value.Size);
    }
    ...
    public Pos X
    {
        ...
        set
        {
            if (!ValidatePosDim(x, value)) throw new ArgumentException();

            x = value;
            SetNeedsLayout();
            if (x is Pos.PosAbsolute) frame = new Rectangle(x.Anchor(0), frame.Y, frame.Width, frame.Height);
    ...
    public Pos Y
    {
        ...
        set
        {
            if (!ValidatePosDim(y, value)) throw new ArgumentException();

            y = value;
            SetNeedsLayout();
            if (y is Pos.PosAbsolute) frame = new Rectangle(frame.X, y.Anchor(0), frame.Width, frame.Height);
            textFormatter.Size = frame.Size;
            SetNeedsDisplay(frame);
        }
    }
    ...
    public Dim Width
    {
        ...
        set
        {
            if (!ValidatePosDim(width, value)) throw new ArgumentException();

            width = value;
            SetNeedsLayout();
            if (width is Dim.DimAbsolute) frame = new Rectangle(frame.X, frame.Y, width.Anchor(0), frame.Height);
            textFormatter.Size = frame.Size;
            SetNeedsDisplay(frame);
        }
    }
    ...
    public Dim Height
    {
        ...
        set
        {
            if (!ValidatePosDim(height, value)) throw new ArgumentException();

            height = value;
            SetNeedsLayout();
            if (height is Dim.DimAbsolute) frame = new Rectangle(frame.X, frame.Y, frame.Width, height.Anchor(0));
            textFormatter.Size = frame.Size;
            SetNeedsDisplay(frame);
        }
    }

    private bool ValidatePosDim(object oldvalue, object newValue)
    {
        if (!IsInitialized || layoutStyle == LayoutStyle.Absolute || oldvalue == null ||
            oldvalue.GetType() == newValue.GetType() || this is Window) return true;
        if (layoutStyle == LayoutStyle.Computed)
            if (oldvalue.GetType() != newValue.GetType() &&
                !(newValue is Pos.PosAbsolute || newValue is Dim.DimAbsolute))
                return true;
        return false;
    }
    ...
    /// </remarks>
    public View()
        : this(string.Empty, TextDirection.LeftRight_TopBottom) { }
    ...
    public View(int x, int y, string text)
        : this(TextFormatter.CalcRect(x, y, text), text) { }
    ...
    public View(Rectangle rect, string text, Border border = null)
    {
    ...
    {
        Initialize(text, Rectangle.Empty, LayoutStyle.Computed, direction, border);
    }
        ...
        Border border = null)
    {
        ...
        Border = border;
        if (Border != null) Border.Child = this;
    ...
    /// </summary>
    public void SetNeedsDisplay() { SetNeedsDisplay(Bounds); }
    ...
    public void SetNeedsDisplay(Rectangle region)
    {
        ...
        else
        {
            ...
            NeedDisplay = new Rectangle(x, y, w, h);
        }
        ...
        foreach (var view in subviews)
            ...
            {
                var childRegion = Rectangle.Intersect(view.Frame, region);
    ...
    {
        if (subviews.Contains(subview)) action(subview);
    ...
    /// </summary>
    internal Rectangle ViewToScreen(Rectangle region)
    {
        ViewToScreen(region.X, region.Y, out var x, out var y, false);
    ...
    internal Rectangle ScreenClip(Rectangle regionScreen)
    {
        ...
        var w = regionScreen.X + regionScreen.Width >= Driver.Cols
            ? Driver.Cols - regionScreen.X
            : regionScreen.Width;
        var h = regionScreen.Y + regionScreen.Height >= Driver.Rows
    ...
    /// </remarks>
    public Rectangle ClipToBounds() { return SetClip(Bounds); }
    ...
    public Rectangle SetClip(Rectangle region)
    {
        var previous = Driver.Clip;
        Driver.Clip = Rectangle.Intersect(previous, ViewToScreen(region));
        return previous;
    }
    ...
    public void DrawFrame(Rectangle region, int padding = 0, bool fill = false)
    {
        var scrRect = ViewToScreen(region);
        var savedClip = ClipToBounds();
        Driver.DrawWindowFrame(scrRect, padding + 1, padding + 1, padding + 1, padding + 1, true, fill);
    ...
    /// </remarks>
    public void DrawHotString(string text, CellAttributes hotColor, CellAttributes normalColor)
    {
    ...
    {
        if (!CanBeVisible(this) || !Enabled) return;

        if (focused?.Visible == true && focused?.Enabled == true && focused?.Frame.Width > 0 &&
    ...
    private void SetHasFocus(bool value, View view)
    {
        if (hasFocus != value)
        {
            hasFocus = value;
    ...
    public virtual void OnAdded(View view) { view.Added?.Invoke(this); }

    /// <summary>
    /// Method invoked when a subview is being removed from this view.
    /// </summary>
    /// <param name="view">The subview being removed.</param>
    public virtual void OnRemoved(View view) { view.Removed?.Invoke(this); }
    ...
    public void AddRune(int col, int row, char ch) { AddRune(col, row, (Rune)ch); }
    ...
    /// <remarks>
    /// <para>
    ///    Always use <see cref="Bounds"/> (view-relative) when calling <see cref="View.Redraw(Rectangle)"/>, NOT <see cref="Frame"/> (superview-relative).
    /// </para>
    /// <para>
    ...
    /// </para>
    /// </remarks>
    public virtual void Redraw(Rectangle bounds)
    {
        if (!CanBeVisible(this)) return;

        var clipRect = new Rectangle(Point.Empty, frame.Size);
        ...
        if (!string.IsNullOrEmpty(Text) || this is Label && !AutoSize)
        {
            Clear();
            // Draw any Text
            if (textFormatter != null) textFormatter.NeedsFormat = true;
            textFormatter?.Draw(
                Application!.Driver,
                ViewToScreen(Bounds),
                HasFocus ? ColorScheme.Focus : GetNormalColor(),
        ...
        if (subviews != null)
            foreach (var view in subviews)
                if (!view.NeedDisplay.IsEmpty || view.ChildNeedsDisplay || view.LayoutNeeded)
                {
                    ...
                    view.NeedDisplay = Rectangle.Empty;
    ...
    /// </para>
    /// </remarks>
    public event Action<Rectangle> DrawContent;
    ...
    /// </remarks>
    public virtual void OnDrawContent(Rectangle viewport) { DrawContent?.Invoke(viewport); }
    ...
    public void SetFocus()
    {
        ...
        {
            if (HasFocus) SetHasFocus(false, this);
            return;
        }
    ...
    /// </summary>
    public class KeyEventEventArgs : System.EventArgs
    {
        /// <summary>
        /// Constructs.
        /// </summary>
    ...
    ///  </para>
    /// </remarks>
    public virtual bool ProcessHotKey(KeyEvent keyEvent)
    {
        if (!Enabled) return false;
    ...
    public virtual bool OnKeyDown(KeyEvent keyEvent)
    {
        ...
        if (Focused?.Enabled == true && Focused?.OnKeyDown(keyEvent) == true) return true;
    ...
    public virtual bool OnKeyUp(KeyEvent keyEvent)
    {
        ...
        KeyUp?.Invoke(args);
        if (args.Handled) return true;
        if (Focused?.Enabled == true && Focused?.OnKeyUp(keyEvent) == true) return true;
    ...
    /// </remarks>
    internal void SetRelativeLayout(Rectangle hostFrame)
    {
        int w, h, _x, _y;
        ...
        var r = new Rectangle(_x, _y, w, h);
        if (Frame != r) Frame = new Rectangle(_x, _y, w, h);
    }

    // https://en.wikipedia.org/wiki/Topological_sorting
    private List<View> TopologicalSort(HashSet<View> nodes, HashSet<(View From, View To)> edges)
    {
        ...
        while (S.Any())
        {
            ...
            foreach (var e in edges.Where(e => e.From.Equals(n)).ToArray())
            {
                ...
                if (edges.All(me => !me.To.Equals(m)) && m != this?.SuperView) // insert m into S
                    S.Add(m);
            }
        }
        ...
        if (edges.Any())
        {
            ...
            if (from != Application.Top)
            {
                if (!ReferenceEquals(from, to))
                    throw new InvalidOperationException(
                        $"TopologicalSort (for Pos/Dim) cannot find {@from} linked with {to}. Did you forget to add it to {this}?");
                else
                    throw new InvalidOperationException(
                        "TopologicalSort encountered a recursive cycle in the relative Pos/Dim in the views of " +
                        this);
            }
        }
    ...
    public virtual void LayoutSubviews()
    {
        ...
        void CollectPos(Pos pos, View from, ref HashSet<View> nNodes, ref HashSet<(View, View)> nEdges)
        {
            ...
            {
                if (pv.Target != this) nEdges.Add((pv.Target, @from));
                foreach (var v in from.InternalSubviews) CollectAll(v, ref nNodes, ref nEdges);
                return;
            }
        ...
        void CollectDim(Dim dim, View from, ref HashSet<View> nNodes, ref HashSet<(View, View)> nEdges)
        {
            ...
            {
                if (dv.Target != this) nEdges.Add((dv.Target, @from));
                foreach (var v in from.InternalSubviews) CollectAll(v, ref nNodes, ref nEdges);
                return;
            }
        ...
        void CollectAll(View from, ref HashSet<View> nNodes, ref HashSet<(View, View)> nEdges)
        {
            ...
            {
                nNodes.Add(v);
                if (v.layoutStyle != LayoutStyle.Computed) continue;
        ...
        {
            if (v.LayoutStyle == LayoutStyle.Computed) v.SetRelativeLayout(Frame);
    ...
    public virtual string Text
    {
        ...
        set
        {
            ...
            if (canResize && textFormatter.Size != Bounds.Size)
                Bounds = new Rectangle(new Point(Bounds.X, Bounds.Y), textFormatter.Size);
            else if (!canResize && textFormatter.Size != Bounds.Size) textFormatter.Size = Bounds.Size;
            SetNeedsLayout();
            SetNeedsDisplay();
        }
    }
    ...
    public virtual TextDirection TextDirection
    {
        ...
        set
        {
            if (textFormatter.Direction != value)
            {
                ...
                {
                    var b = new Rectangle(Bounds.X, Bounds.Y, Bounds.Height, Bounds.Width);
                    SetWidthHeight(b);
                }
    ...
    private bool ResizeView(bool autoSize)
    {
        ...
        var aSize = autoSize;
        var nBounds = TextFormatter.CalcRect(Bounds.X, Bounds.Y, Text, textFormatter.Direction);
        if (textFormatter.Size != nBounds.Size) textFormatter.Size = nBounds.Size;
        if ((textFormatter.Size != Bounds.Size || textFormatter.Size != nBounds.Size)
            && ((width == null || width is Dim.DimAbsolute) && (Bounds.Width == 0
                ...
                || (height == null || height is Dim.DimAbsolute) && (Bounds.Height == 0
                                                                     || autoSize &&
                                                                     Bounds.Height != nBounds.Height)))
            aSize = SetWidthHeight(nBounds);
        return aSize;
    }

    private bool SetWidthHeight(Rectangle nBounds)
    {
        var aSize = false;
        var canSizeW = SetWidth(nBounds.Width, out var rW);
        var canSizeH = SetHeight(nBounds.Height, out var rH);
        if (canSizeW)
        {
            aSize = true;
            width = rW;
    ...
    /// </summary>
    public virtual void OnCanFocusChanged() { CanFocusChanged?.Invoke(); }

    /// <summary>
    /// Method invoked when the <see cref="Enabled"/> property from a view is changed.
    /// </summary>
    public virtual void OnEnabledChanged() { EnabledChanged?.Invoke(); }

    /// <summary>
    /// Method invoked when the <see cref="Visible"/> property from a view is changed.
    /// </summary>
    public virtual void OnVisibleChanged() { VisibleChanged?.Invoke(); }
    ...
    private bool CanSetWidth(int desiredWidth, out int resultWidth)
    {
        var w = desiredWidth;
        bool canSetWidth;
        ...
        else if (Width is Dim.DimFactor factor)
        {
            ...
            if (factor.IsFromRemaining()) sw -= Frame.X;
            w = Width.Anchor(sw);
            canSetWidth = false;
        }
        else
    ...
    private bool CanSetHeight(int desiredHeight, out int resultHeight)
    {
        var h = desiredHeight;
        bool canSetHeight;
        ...
        else if (Height is Dim.DimFactor factor)
        {
            ...
            if (factor.IsFromRemaining()) sh -= Frame.Y;
            h = Height.Anchor(sh);
            canSetHeight = false;
        }
        else
    ...
    public CellAttributes GetNormalColor() { return Enabled ? ColorScheme.Normal : ColorScheme.Disabled; }

...
public struct KeyModifiers
{
    ...
    public void Clear()
    {
        Shift = Alt = Ctrl = Capslock = Numlock = Scrolllock = false;
    }
}
...
public class KeyEvent
{
    ...
    /// </summary>
    public KeyEvent(Key k, KeyModifiers? km = null)
    {
        Key = k;
        keyModifiers = km ?? new();
    }
    ...
    public override string ToString()
    {
        string msg = "";
        var key = Key;
        if (keyModifiers.Shift) msg += "Shift-";
        if (keyModifiers.Alt) msg += "Alt-";
        if (keyModifiers.Ctrl) msg += "Ctrl-";
        if (keyModifiers.Capslock) msg += "Capslock-";
        if (keyModifiers.Numlock) msg += "Numlock-";
        if (keyModifiers.Scrolllock) msg += "Scrolllock-";

        msg += $"{(((uint)KeyValue & (uint)Key.CharMask) > 27 ? $"{(char)KeyValue}" : $"{key}")}";

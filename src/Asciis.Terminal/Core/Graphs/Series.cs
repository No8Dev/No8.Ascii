...
public class ScatterSeries : ISeries
{
    ...
    /// </summary>
    public GraphCellToRender Fill { get; set; } = new((Rune)'x');
    ...
    public void DrawSeries(GraphView graph, Rectangle drawBounds, RectangleF graphBounds)
    {
        if (Fill.Color.HasValue) 
            graph.Application.Driver.SetAttribute(Fill.Color.Value);
...
public class MultiBarSeries : ISeries
{
    ...
    public MultiBarSeries(int numberOfBarsPerCategory, float barsEvery, float spacing, CellAttributes[] colors = null)
    {
        ...
        for (var i = 0; i < numberOfBarsPerCategory; i++)
        {
            ...
            if (colors != null) subSeries[i].OverrideBarColor = colors[i];
        }
    ...
    public void DrawSeries(GraphView graph, Rectangle drawBounds, RectangleF graphBounds)
    {
        foreach (var bar in subSeries) bar.DrawSeries(graph, drawBounds, graphBounds);
    }
}
...
public class BarSeries : ISeries
{
    ...
    /// </summary>
    public CellAttributes? OverrideBarColor { get; set; }
    ...
    {
        if (OverrideBarColor.HasValue) graphCellToRender.Color = OverrideBarColor;
    ...
    public virtual void DrawSeries(GraphView graph, Rectangle drawBounds, RectangleF graphBounds)
    {
        ...
        {
            var xStart = Orientation == Orientation.Horizontal ? 0 : Offset + (i + 1) * BarEvery;
            var yStart = Orientation == Orientation.Horizontal ? Offset + (i + 1) * BarEvery : 0;

            var endX = Orientation == Orientation.Horizontal ? Bars[i].Value : xStart;
            var endY = Orientation == Orientation.Horizontal ? yStart : Bars[i].Value;
            ...
            if (Orientation == Orientation.Horizontal)
            {
                ...
                if (screenStart.Y < 0 || screenStart.Y > drawBounds.Height - graph.MarginBottom) continue;
            }
            else
            {
                // Start the axis
                ...
                // if bar is off the screen
                if (screenStart.X < graph.MarginLeft ||
                    screenStart.X > graph.MarginLeft + drawBounds.Width - 1) continue;
            }

            // draw the bar unless it has no height
            if (Bars[i].Value != 0) DrawBarLine(graph, screenStart, screenEnd, Bars[i]);
    ...
    protected virtual void DrawBarLine(GraphView graph, Point start, Point end, Bar beingDrawn)
    {
        ...
        if (adjusted.Color.HasValue) 
            graph.Application.Driver.SetAttribute(adjusted.Color.Value);

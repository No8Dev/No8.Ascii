...
public class TextAnnotation : IAnnotation
{
    ...
    protected void DrawText(GraphView graph, int x, int y)
    {
        ...
        if (!graph.Bounds.Contains(new Point(x, y))) return;
        ...
        var availableWidth = graph.Bounds.Width - x;
        ...
        if (Text.Length < availableWidth)
            ...
            graph.Application.Driver.AddStr(Text.Substring(0, availableWidth));
    }
}
...
public class LegendAnnotation : IAnnotation
{
    ...
    /// </summary>
    private List<Tuple<GraphCellToRender, string>> entries = new();
    ...
    public LegendAnnotation(Rectangle legendBounds) { Bounds = legendBounds; }
    ...
    public void Render(GraphView graph)
    {
        if (Border) graph.DrawFrame(Bounds, 0, true);

        // start the legend at
        var y = Bounds.Top + (Border ? 1 : 0);
        var x = Bounds.Left + (Border ? 1 : 0);

        // how much horizontal space is available for writing legend entries?
        var availableWidth = Bounds.Width - (Border ? 2 : 0);
        var availableHeight = Bounds.Height - (Border ? 2 : 0);
        ...
        foreach (var entry in entries)
        {
            if (entry.Item1.Color.HasValue)
                graph.Application.Driver.SetAttribute(entry.Item1.Color.Value);
            else
            ...
            graph.Application.Driver.AddStr(str);

            linesDrawn++;

            // Legend has run out of space
            if (linesDrawn >= availableHeight) break;
        }
    }
...
public class PathAnnotation : IAnnotation
{
    ...
    /// </summary>
    public CellAttributes? LineColor { get; set; }

    /// <summary>
    /// The symbol that gets drawn along the line, defaults to '.'
    /// </summary>
    public Rune LineRune { get; set; } = (Rune)'.';
    ...
    public void Render(GraphView graph)
    {
        graph.Application.Driver.SetAttribute(LineColor ?? graph.ColorScheme.Normal);
    ...
    {
        for (var i = 0; i < Points.Count - 1; i++) yield return new LineF(Points[i], Points[i + 1]);
    }

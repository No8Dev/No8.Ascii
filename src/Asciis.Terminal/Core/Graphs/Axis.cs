namespace Asciis.Terminal.Core.Graphs;
...
public class HorizontalAxis : Axis
{
    ...
    public override void DrawAxisLine(GraphView graph)
    {
        ...
        for (var i = xStart; i < bounds.Width; i++) DrawAxisLine(graph, i, y);
    }
    ...
    {
        graph.Move(x, y);
        graph.Application.Driver.AddRune(graph.Application.Driver.HLine);
    }
    ...
    {
        if (!Visible || Increment == 0) return;

        var bounds = graph.Bounds;

        var labels = GetLabels(graph, bounds);

        foreach (var label in labels) DrawAxisLabel(graph, label.ScreenLocation, label.Text);
        ...
        if (!string.IsNullOrWhiteSpace(Text))
        {
            ...
            if (toRender.Length > graph.Bounds.Width) toRender = toRender.Substring(0, graph.Bounds.Width);

            graph.Move(graph.Bounds.Width / 2 - toRender.Length / 2, graph.Bounds.Height - 1);
            graph.Application.Driver.AddStr(toRender);
        }
    }
    ...
    {
        var driver = graph.Application.Driver;
        var y = GetAxisYPosition(graph);
        ...
        if (!string.IsNullOrWhiteSpace(text))
        {
            ...
            var drawAtX = Math.Max(0, screenPosition - text.Length / 2);
            string toRender = text;

            // this is how much space is left
            var xSpaceAvailable = graph.Bounds.Width - drawAtX;

            // There is no space for the label at all!
            if (xSpaceAvailable <= 0) return;

            // if we are close to right side of graph, don't overspill
            if (toRender.Length > xSpaceAvailable) toRender = toRender.Substring(0, xSpaceAvailable);
    ...
    private IEnumerable<AxisIncrementToRender> GetLabels(GraphView graph, Rectangle bounds)
    {
        ...
        if (Minimum.HasValue) start.X = Math.Max(start.X, Minimum.Value);

        var current = start;

        while (current.X < end.X)
        {
            var screenX = graph.GraphSpaceToScreen(new PointF(current.X, current.Y)).X;
            ...
            if (screenX >= graph.MarginLeft)
            {
                ...
                if (ShowLabelsEvery != 0)
                {
                    ...
                    if (labels++ % ShowLabelsEvery == 0) toRender.Text = LabelGetter(toRender);
                    ;
                }
...
public class VerticalAxis : Axis
{
    ...
    public override void DrawAxisLine(GraphView graph)
    {
        ...
        // Draw solid line
        for (var i = 0; i < yEnd; i++) DrawAxisLine(graph, x, i);
    }
    ...
    {
        graph.Move(x, y);
        graph.Application.Driver.AddRune(graph.Application.Driver.VLine);
    }
    ...
    private int GetAxisYEnd(GraphView graph)
    {
        ...
        if (Minimum.HasValue) return graph.GraphSpaceToScreen(new PointF(0, Minimum.Value)).Y;
    ...
    public override void DrawAxisLabels(GraphView graph)
    {
        ...
        if (!string.IsNullOrWhiteSpace(Text))
        {
            ...
            // if label is too long
            if (toRender.Length > graph.Bounds.Height) toRender = toRender.Substring(0, graph.Bounds.Height);

            // Draw it 1 letter at a time vertically down row 0 of the control
            var startDrawingAtY = graph.Bounds.Height / 2 - toRender.Length / 2;

            for (var i = 0; i < toRender.Length; i++)
            {
                graph.Move(0, startDrawingAtY + i);
                graph.Application.Driver.AddRune(toRender[i]);
            }
        }
    }
    ...
    private IEnumerable<AxisIncrementToRender> GetLabels(GraphView graph, Rectangle bounds)
    {
        ...
        // don't draw labels below the minimum
        if (Minimum.HasValue) start.Y = Math.Max(start.Y, Minimum.Value);
        ...
        {
            var screenY = graph.GraphSpaceToScreen(new PointF(current.X, current.Y)).Y;
    ...
    public override void DrawAxisLabel(GraphView graph, int screenPosition, string text)
    {
        ...
        // draw the tick on the axis
        graph.Application.Driver.AddRune(graph.Application.Driver.RightTee);

        // and the label text
        if (!string.IsNullOrWhiteSpace(text))
        {
            graph.Move(Math.Max(0, x - labelThickness), screenPosition);
            graph.Application.Driver.AddStr(text);
        }
    }

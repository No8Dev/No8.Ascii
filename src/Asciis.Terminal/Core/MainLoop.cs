...
public class MainLoop
{
    ...
    private SortedList<long, Timeout> timeouts = new();
    private List<Func<bool>> idleHandlers = new();
    private object idleHandlersLock = new();
    ...
    private void AddTimeout(TimeSpan time, Timeout timeout)
    {
        ...
        lock (timeouts)
        {
            while (timeouts.ContainsKey(k)) k = (DateTime.UtcNow + time).Ticks;
    ...
    public bool RemoveTimeout(object token)
    {
        ...
        {
            var idx = timeouts.IndexOfValue(timeout);
            if (idx == -1)
                return false;
    ...
    public long FirstTimeout()
    {
        if (timeouts.Count > 0)
            return timeouts.Keys[0];
    ...
    private void RunTimers()
    {
        ...
        var copy = new SortedList<long, Timeout>(timeouts);
        ...
        foreach (var t in copy)
        {
            ...
            if (k < now)
            {
                if (timeout.Callback?.Invoke(this) == true)
    ...
    /// </remarks>
    public bool EventsPending(bool wait = false) { return Driver.EventsPending(wait); }

...
public class MenuItem
{
    ...
    public MenuItem(Key shortcut = Key.Null)
    {
        ...
        if (shortcut != Key.Null) shortcutHelper.Shortcut = shortcut;
    }
    ...
    internal int Width =>
        Title.RuneCount() + Help.RuneCount() + 1 + 2 +
        (Checked || CheckType.HasFlag(MenuItemCheckStyle.Checked) || CheckType.HasFlag(MenuItemCheckStyle.Radio)
            ? 2
            : 0) +
    ...
    private void GetHotKey()
    {
        ...
        foreach (var x in title)
            ...
            else
            {
                if (nextIsHot)
                {
                    HotKey = Rune.ToUpperInvariant((Rune)x);
                    break;
                }
...
public class MenuBarItem : MenuItem
{
    ...
    private void SetChildrensParent(MenuItem[] childrens)
    {
    ...
    public MenuBarItem SubMenu(MenuItem children) { return children as MenuBarItem; }
...
internal class Menu : View
{
    ...
    private static Rectangle MakeFrame(int x, int y, MenuItem[] items)
    {
        if (items == null || items.Length == 0) return new Rectangle();
        var maxW = items.Max(z => z?.Width) ?? 0;

        return new Rectangle(x, y, maxW + 2, items.Length + 2);
    }

    public Menu(MenuBar host, int x, int y, MenuBarItem barItems)
        : base(MakeFrame(x, y, barItems.Children))
    {
    ...
    internal CellAttributes DetermineColorSchemeFor(MenuItem item, int index)
    {
        if (item != null)
        {
    ...
    public override void Redraw(Rectangle bounds)
    {
        ...
        for (var i = 0; i < barItems.Children.Length; i++)
        {
            ...
            if (item.Checked)
                textToDraw = new Rune[] { checkChar, (Rune)' ' }.MakeString() + item.Title;
            else if (item.CheckType.HasFlag(MenuItemCheckStyle.Checked) ||
                     item.CheckType.HasFlag(MenuItemCheckStyle.Radio))
                textToDraw = new Rune[] { uncheckedChar, (Rune)' ' }.MakeString() + item.Title;
            else
            ...
            // The help string
            var l = item.ShortcutTag.RuneCount() == 0
                ? item.Help.RuneCount()
                : item.Help.RuneCount() + item.ShortcutTag.RuneCount() + 2;
    ...
    public override bool OnLeave(View view) { return host.OnLeave(view); }
    ...
    public override bool ProcessKey(KeyEvent kb)
    {
        switch (kb.Key)
        {
            ...
            case Key.CursorRight:
                host.NextMenu(
                    barItems.IsTopLevel || barItems.Children != null && current > -1 &&
                    current < barItems.Children.Length && barItems.Children[current].IsFromSubMenu
                        ? true
                        : false);
                return true;
            case Key.Esc:
            ...
            case Key.Enter:
                if (barItems.IsTopLevel)
                    Run(barItems.Action);
                else if (current > -1) Run(barItems.Children[current].Action);
                return true;
            default:
                // TODO: rune-ify
                if (barItems.Children != null && char.IsLetterOrDigit((char)kb.KeyValue))
                {
                    ...
                    foreach (var item in barItems.Children)
                    {
                        ...
                        if (item.IsEnabled() && item.HotKey == (Rune)x)
                        {
                            host.CloseMenu();
                            Run(item.Action);
                            return true;
    ...
    {
        if (barItems.IsTopLevel) return true;
        bool disabled;
        do
        {
            current++;
            if (current >= barItems.Children.Length) current = 0;
            ...
            if (!host.IsMenuOpen) host.OpenMenu(host.selected);
    ...
    private bool MoveUp()
    {
        if (barItems.IsTopLevel || current == -1) return true;
        bool disabled;
        do
        {
            current--;
            if (host.UseKeysUpDownAsKeysLeftRight)
                if ((current == -1 || this != host.openCurrentMenu) &&
                    barItems.Children[current + 1].IsFromSubMenu && host.selectedSub > -1)
                {
                    current++;
                    host.PreviousMenu(true);
                    if (current > 0)
    ...
    {
        if (!host.handled && !host.HandleGrabView(me, this)) return false;
        host.handled = false;
        bool disabled;
        ...
        {
            disabled = false;
            if (me.Y < 1 || me.Y - 1 >= barItems.Children.Length) return true;
    ...
    {
        if (current == -1 || barItems.Children[current] == null) return;
        var subMenu = barItems.SubMenu(barItems.Children[current]);
        if (subMenu != null)
        {
            var pos = -1;
            if (host.openSubMenu != null) pos = host.openSubMenu.FindIndex(o => o?.barItems == subMenu);
            if (pos == -1 && this != host.openCurrentMenu &&
                subMenu.Children != host.openCurrentMenu.barItems.Children) host.CloseMenu(false, true);
...
public class MenuBar : View
{
    ...
    private static string shortcutDelimiter = "+";
    ...
    public static string ShortcutDelimiter
    {
        ...
        set
        {
            if (shortcutDelimiter != value) shortcutDelimiter = value == string.Empty ? " " : value;
        }
    }
    ...
    {
        if ((!(view is MenuBar) && !(view is Menu) || !(view is MenuBar) && !(view is Menu) && openMenu != null) &&
            !isCleaning && !reopen)
        {
            CleanUp();
            return true;
    ...
    public override bool OnKeyUp(KeyEvent keyEvent)
    {
        if (keyEvent.IsAlt || keyEvent.IsCtrl && keyEvent.Key == (Key.CtrlMask | Key.Space))
        {
            ...
            if (openedByAltKey && !IsMenuOpen && openMenu == null && (((uint)keyEvent.Key & (uint)Key.CharMask) == 0
                                                                      || ((uint)keyEvent.Key &
                                                                          (uint)Key.CharMask) == (uint)Key.Space))
            {
    ...
    {
        isCleaning = true;
        if (openMenu != null) CloseAllMenus();
        openedByAltKey = false;
        IsMenuOpen = false;
        selected = -1;
        CanFocus = false;
        if (lastFocused != null) lastFocused.SetFocus();
        SetNeedsDisplay();
        Application.UngrabMouse();
        isCleaning = false;
    }
    ...
    public override void Redraw(Rectangle bounds)
    {
        ...
        for (var i = 0; i < Menus.Length; i++)
        {
            var menu = Menus[i];
            Move(pos, 0);
            CellAttributes hotColor, normalColor;
            ...
            DrawHotString(
                menu.Help.IsEmpty() ? $" {menu.Title}  " : $" {menu.Title}  {menu.Help}  ",
                hotColor,
                normalColor);
    ...
    public override void PositionCursor()
    {
        if (selected == -1 && HasFocus && Menus.Length > 0) selected = 0;
        var pos = 0;
        for (var i = 0; i < Menus.Length; i++)
            if (i == selected)
            {
    ...
    /// </summary>
    public virtual void OnMenuClosing() { MenuClosing?.Invoke(); }
        ...
        switch (subMenu)
        {
            case null:
                ...
                for (var i = 0; i < index; i++)
                    pos += Menus[i].Title.RuneCount() +
                           (Menus[i].Help.RuneCount() > 0 ? Menus[i].Help.RuneCount() + 2 : 0) + 2;
            ...
            default:
                ...
                else
                {
                    ...
                    openCurrentMenu = new Menu(
                        this,
                        last.Frame.Left + last.Frame.Width,
                        last.Frame.Top + 1 + last.current,
                        subMenu);
                ...
                if (selectedSub > -1 && SelectEnabledItem(
                    openCurrentMenu.barItems.Children,
                    openCurrentMenu.current,
                    out openCurrentMenu.current)) openCurrentMenu.SetFocus();
                break;
        }
    ...
    public void OpenMenu()
    {
        ...
        if (!SelectEnabledItem(
            openCurrentMenu.barItems.Children,
            openCurrentMenu.current,
            out openCurrentMenu.current)) CloseMenu();
    ...
    /// </summary>
    public void CloseMenu() { CloseMenu(false, false); }
    ...
    internal void CloseMenu(bool reopen = false, bool isSubMenu = false)
    {
        ...
        {
            case false:
                if (openMenu != null) SuperView?.Remove(openMenu);
                SetNeedsDisplay();
                if (previousFocused != null && previousFocused is Menu && openMenu != null &&
                    previousFocused.ToString() != openCurrentMenu.ToString())
                    previousFocused.SetFocus();
                openMenu?.Dispose();
                openMenu = null;
                if (lastFocused is Menu || lastFocused is MenuBar) lastFocused = null;
                LastFocused = lastFocused;
                lastFocused = null;
                if (LastFocused != null && LastFocused.CanFocus)
                {
                    if (!reopen) selected = -1;
    ...
    private void RemoveSubMenu(int index)
    {
        if (openSubMenu == null)
            return;
        for (var i = openSubMenu.Count - 1; i > index; i--)
        {
            isMenuClosing = true;
    ...
    private View FindDeepestMenu(View view, ref int count)
    {
        ...
        switch (isSubMenu)
        {
            ...
            case true:
                ...
                else
                {
                    ...
                    if ((selectedSub == -1 || openSubMenu == null || openSubMenu?.Count == selectedSub) &&
                        subMenu == null)
                    {
                        if (openSubMenu != null)
                            CloseMenu(false, true);
    ...
    {
        if (children == null) children = Menus;
        ...
        for (var i = 0; i < children.Length; i++)
        {
            var mi = children[i];
            if (mi == null) continue;
            if ((!(mi is MenuBarItem mbiTopLevel) || mbiTopLevel.IsTopLevel) && mi.Shortcut != Key.Null &&
                mi.Shortcut == (Key)key)
            {
            ...
            if (mi is MenuBarItem menuBarItem && !menuBarItem.IsTopLevel &&
                FindAndOpenMenuByShortcut(kb, menuBarItem.Children)) return true;
        }
    ...
    public override bool ProcessKey(KeyEvent kb)
    {
        switch (kb.Key)
        {
            ...
            case Key.CursorDown:
            case Key.Enter:
                if (selected > -1) ProcessMenu(selected, Menus[selected]);
                break;
    ...
    ///<inheritdoc/>
    public override bool ProcessColdKey(KeyEvent kb) { return FindAndOpenMenuByShortcut(kb); }

    ///<inheritdoc/>
    public override bool MouseEvent(MouseEvent me)
    {
        if (!handled && !HandleGrabView(me, this)) return false;
        handled = false;
                ...
                if (cx >= pos && cx < pos + 1 + Menus[i].TitleLength + Menus[i].Help.RuneCount() + 2)
                {
                             ...
                             me.Flags == MouseFlags.Button1TripleClicked)
                    {
                        if (IsMenuOpen && !Menus[i].IsTopLevel)
                            CloseAllMenus();
                        else if (!Menus[i].IsTopLevel) Activate(i);
                    }
        ...
        if (Application.mouseGrabView != null)
        {
            ...
            else
            {
                ...
                return false;
                                 ...
                                 me.Flags == MouseFlags.Button1DoubleClicked ||
                                 me.Flags == MouseFlags.Button1TripleClicked || me.Flags.HasFlag(
...
/// <summary>
/// An <see cref="System.EventArgs"/> which allows passing a cancelable menu opening event or replacing with a new <see cref="MenuBarItem"/>.
/// </summary>

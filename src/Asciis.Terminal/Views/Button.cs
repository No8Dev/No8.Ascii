...
public class Button : View
{
    ...
    /// </param>
    public Button(string text, bool is_default = false)
        : base(text)
    {
    ...
    public Button(int x, int y, string text, bool is_default)
        : base(new Rectangle(x, y, text.RuneCount() + 4 + (is_default ? 2 : 0), 1), text)
    {
        Init(text, is_default);
    }
    ...
    private void Init(string text, bool is_default)
    {
        TextAlignment = TextAlignment.Centered;

        HotKeySpecifier = (Rune)'_';

        _leftBracket = Driver != null ? Driver.LeftBracket : (Rune)'[';
        _rightBracket = Driver != null ? Driver.RightBracket : (Rune)']';
        _leftDefault = Driver != null ? Driver.LeftDefaultIndicator : (Rune)'<';
        _rightDefault = Driver != null ? Driver.RightDefaultIndicator : (Rune)'>';
    ...
    {
        if (IsDefault)
            base.Text = _leftBracket.ToString() + _leftDefault.ToString() + " " + text + " " +
                        _rightDefault.ToString() + _rightBracket.ToString();
        else
            base.Text = _leftBracket.ToString() + " " + text + " " + _rightBracket.ToString();

        var w = base.Text.RuneCount() - (base.Text.ToRuneList().Contains(HotKeySpecifier) ? 1 : 0);
        GetCurrentWidth(out var cWidth);
        var canSetWidth = SetWidth(w, out var rWidth);
        ...
        base.Frame = new Rectangle(base.Frame.Location, new Size(w, 1));
        SetNeedsDisplay();
    }

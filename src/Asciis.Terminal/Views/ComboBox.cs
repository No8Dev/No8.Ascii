...
public class ComboBox : View
{
    ...
    private readonly IList searchset = new List<object>();
    private string text = "";
    ...
    public ComboBox(Rectangle rect, IList source)
        : base(rect)
    {
    ...
    {
        if (Bounds.Height < minimumHeight && Height is Dim.DimAbsolute) Height = minimumHeight;
        ...
        {
            if (searchset.Count > 0) SetValue(searchset[listview.SelectedItem]);
        };
    ...
    public override bool MouseEvent(MouseEvent me)
    {
        ...
        {
            if (!search.HasFocus) search.SetFocus();
    ...
    {
        if (!search.HasFocus && !listview.HasFocus) search.SetFocus();
    ...
    public override bool ProcessKey(KeyEvent e)
    {
        ...
        if (e.Key == Key.CursorUp && listview.HasFocus && listview.SelectedItem == 0 &&
            searchset?.Count > 0) // jump back to search
        {
            search.CursorPosition = search.Text.RuneCount();
            search.SetFocus();
            return true;
        ...
        {
            if (listview.SelectedItem != -1) listview.MovePageDown();
            return true;
        }

        if (e.Key == Key.PageUp)
        {
            if (listview.SelectedItem != -1) listview.MovePageUp();
            return true;
        }

        if (e.Key == Key.Home)
        {
            if (listview.SelectedItem != -1) listview.MoveHome();
            return true;
        }

        if (e.Key == Key.End)
        {
            if (listview.SelectedItem != -1) listview.MoveEnd();
            return true;
        }
    ...
    {
        if (!keepSearchText) search.Text = text = "";
        ...
        if (Subviews.Count > 0) search.SetFocus();
    }
    ...
    {
        // force deep copy
        foreach (var item in Source.ToList()) searchset.Add(item);
    }
    ...
    private int CalculatetHeight()
    {
        ...
        return Math.Min(
            Math.Max(Bounds.Height - 1, minimumHeight - 1),
            searchset?.Count > 0 ? searchset.Count : isShow ? Math.Max(Bounds.Height - 1, minimumHeight - 1) : 0);
    }
}

...
public class TextField : View
{
    ...
    private int selectedStart = -1; // -1 represents there is no text selection.
    ...
    /// </remarks>
    /// <remarks>
    ///   The passed <see cref="System.EventArgs"/> is a <see cref="string"/> containing the old value. 
    /// </remarks>
    ...
    /// <param name="text">Initial text contents.</param>
    public TextField(int x, int y, int w, string text)
        : base(new Rectangle(x, y, w, 1))
    {
        Initialize(text, w);
    }

    private void Initialize(string text, int w)
    {
        Height = 1;
    ...
    public new string Text
    {
        ...
        set
        {
            ...
            if (newText.Cancel)
            {
                if (point > text.Count) point = text.Count;
                return;
            }
            ...
            {
                if (historyText == null)
                    historyText = new List<string>() { oldText };
                ...
                historyText.Add(text.MakeString());
                idxhistoryText++;
            }

            TextChanged?.Invoke(oldText);

            if (point > text.Count) point = Math.Max(TextModel.DisplaySize(text, 0).size - 1, 0);
    ...
    {
        var selColor = new CellAttributes(ColorScheme.Focus.Background, ColorScheme.Focus.Foreground);
        ...
        for (var idx = p; idx < tcount; idx++)
        {
            ...
            if (idx == point && HasFocus && !Used && length == 0 && !ReadOnly)
                ...
                Driver.SetAttribute(
                    idx >= start && length > 0 && idx < start + length ? selColor : ColorScheme.Focus);
            if (col + cols <= width) Driver.AddRune((Rune)(Secret ? (Rune)'*' : rune));
            if (!TextModel.SetCol(ref col, width, cols)) break;
            if (idx + 1 < tcount && col + text[idx + 1].ColumnWidth() > width) break;
        }
    ...
    private CellAttributes GetReadOnlyColor()
    {
        if (ColorScheme.Disabled.Foreground == ColorScheme.Focus.Background)
            return new CellAttributes(ColorScheme.Focus.Foreground, ColorScheme.Focus.Background);
        return new CellAttributes(ColorScheme.Disabled.Foreground, ColorScheme.Focus.Background);
    }
    ...
    private void SetText(List<Rune> newText) { Text = newText.MakeString(); }
    ...
    {
        if (!Secret)
            Application.Driver.Clipboard.Contents = text.ToList().MakeString();
    }
    ...
    public override bool ProcessKey(KeyEvent kb)
    {
        ...
        switch (ShortcutHelper.GetModifiersKey(kb))
        {
            ...
            case Key.Backspace:
                ...
                if (length == 0)
                {
                    ...
                    if (oldCursorPos < text.Count)
                        SetText(
                            text.GetRange(0, oldCursorPos - 1)
            ...
            case (Key)(((int)'B' + Key.ShiftMask) | Key.AltMask):
                if (point > 0)
                {
                    var x = Math.Min(start > -1 && start > point ? start : point, text.Count);
                    if (x > 0)
                    {
            ...
            default:
                ...
                var kbstr = new List<Rune> { (Rune)(uint)kb.Key };
                if (Used)
                {
                    point++;
                    ...
                    else
                    {
                        ...
                        SetText(
                            text.GetRange(0, oldCursorPos)
                                .Concat(kbstr)
                                .Concat(text.GetRange(oldCursorPos, Math.Min(text.Count - oldCursorPos, text.Count))));
                    }
                }
                else
                {
                    SetText(
                        text.GetRange(0, oldCursorPos)
                            ...
                            .Concat(
                                 text.GetRange(
                                     Math.Min(oldCursorPos + 1, text.Count),
    ...
    private int WordBackward(int p)
    {
        ...
        if (Rune.IsPunctuation(ti) || Rune.IsSymbol(ti) || Rune.IsWhiteSpace(ti))
        {
            for (; i >= 0; i--)
            {
                ...
                if (i - 1 > 0 && !Rune.IsWhiteSpace(text[i]) && Rune.IsWhiteSpace(text[i - 1])) return i;
            }
            ...
            if (lastValidCol > -1) return lastValidCol;
        }
        else
        {
            for (; i >= 0; i--)
    ...
    /// </summary>
    public int SelectedLength => length;
    ...
    {
        var pX = x;
        if (getX) pX = TextModel.GetColFromX(text, first, x);
    ...
    private void PrepareSelection(int x, int direction = 0)
    {
        ...
        {
            length = Math.Abs(
                x + direction <= text.Count ? x + direction - selectedStart : text.Count - selectedStart);
            SetSelectedStartSelectedLength();
            if (start > -1 && length > 0)
            {
                selectedText = length > 0
                    ? text.MakeString()
                          .Substring(
                               start < 0 ? 0 : start,
                               length > text.Count ? text.Count : length)
                    : "";
    ...
    public virtual void Copy()
    {
        ...
        Application.Driver.Clipboard.Contents = SelectedText;
    }
    ...
    private void DeleteSelectedText()
    {
        ...
        SetSelectedStartSelectedLength();
        var selStart = SelectedStart > -1 ? start : point;
        var (_, len) = TextModel.DisplaySize(text, 0, selStart, false);
        var (_, len2) = TextModel.DisplaySize(text, selStart, selStart + length, false);
        var (_, len3) = TextModel.DisplaySize(text, selStart + length, actualText.RuneCount(), false);
        Text = actualText.Substring(0, len) +
    ...
    {
        if (ReadOnly || Application.Driver.Clipboard.Contents == null) return;
        ...
        var (_, len2) = TextModel.DisplaySize(text, selStart, selStart + length, false);
        var (_, len3) = TextModel.DisplaySize(text, selStart + length, actualText.RuneCount(), false);
        string cbTxt = Application.Driver.Clipboard.Contents.Split("\n")[0] ?? "";
        Text = actualText.Substring(0, len) +
               cbTxt +
               actualText.Substring(len + len2, len3);
...
/// <summary>
/// An <see cref="System.EventArgs"/> which allows passing a cancelable new text value event.
/// </summary>

...
public class ScrollBarView : View
{
    ...
    public ScrollBarView(Rectangle rect, int size, int position, bool isVertical)
        : base(rect)
    {
    ...
    /// <summary>
    /// Initializes a new instance of the <see cref="ScrollBarView"/> class using <see cref="LayoutStyle.Computed"/> layout.
    /// </summary>
    ...
    private void Host_VisibleChanged()
    {
        ...
        {
            Visible = Host.Visible;
            if (otherScrollBarView != null) otherScrollBarView.Visible = Visible;
    ...
    {
        Enabled = Host.Enabled;
        if (otherScrollBarView != null) otherScrollBarView.Enabled = Enabled;
    ...
    {
        CanFocus = Host.CanFocus;
        if (otherScrollBarView != null) otherScrollBarView.CanFocus = CanFocus;
    }
    ...
    private void Init(int size, int position, bool isVertical)
    {
        vertical = isVertical;
    ...
    public int Position
    {
        ...
        set
        {
            if (position != value)
            {
                if (CanScroll(value - position, out var max, vertical))
                {
        ...
        set
        {
            if (value != null && (value.IsVertical && vertical || !value.IsVertical && !vertical))
                throw new ArgumentException(
                    $"There is already a {(vertical ? "vertical" : "horizontal")} ScrollBarView.");
    ...
    public bool ShowScrollIndicator
    {
        ...
        set
        {
            if (value == showScrollIndicator) return;
    ...
    {
        get => keepContentAlwaysInViewport;
        set
        {
    ...
    /// </summary>
    public virtual void OnChangedPosition() { ChangedPosition?.Invoke(); }
    ...
    private void ShowHideScrollBars()
    {
        ...
        if (otherScrollBarView != null) CheckBothScrollBars(otherScrollBarView, pending);

        SetWidthHeight();
        SetRelativeLayout(Bounds);
        if (otherScrollBarView != null) OtherScrollBarView.SetRelativeLayout(OtherScrollBarView.Bounds);

        if (showBothScrollIndicator)
        {
            if (contentBottomRightCorner != null) contentBottomRightCorner.Visible = true;
        }
        else if (!showScrollIndicator)
        {
            if (contentBottomRightCorner != null) contentBottomRightCorner.Visible = false;
        ...
        if (Host?.Visible == true && showScrollIndicator && !Visible) Visible = true;
        ...
        if (showScrollIndicator) Redraw(Bounds);
    ...
    {
        var barsize = scrollBarView.vertical ? scrollBarView.Bounds.Height : scrollBarView.Bounds.Width;

        if (barsize == 0 || barsize > scrollBarView.size)
        {
            if (scrollBarView.showScrollIndicator) scrollBarView.ShowScrollIndicator = false;
        }
        else if (barsize > 0 && barsize == scrollBarView.size && scrollBarView.OtherScrollBarView != null &&
                 pending)
        {
            if (scrollBarView.showScrollIndicator) scrollBarView.ShowScrollIndicator = false;
        ...
        else
        {
            ...
            if (!scrollBarView.showScrollIndicator) scrollBarView.ShowScrollIndicator = true;
        }
    ...
    private int posTopTee;
    private int posLeftTee;
    private int posBottomTee;
    private int posRightTee;

    ///<inheritdoc/>
    public override void Redraw(Rectangle region)
    {
        if (ColorScheme == null || Size == 0) return;

        Driver.SetAttribute(GetNormalColor());

        if (vertical && Bounds.Height == 0 || !vertical && Bounds.Width == 0) return;

        if (vertical)
        {
            if (region.Right < Bounds.Width - 1) return;
            ...
            else
            {
                ...
                var by2 = KeepContentAlwaysInViewport
                    ? Math.Min((position + bh) * bh / Size + 1, bh - 1)
                    : (position + bh) * bh / (Size + bh);
                if (KeepContentAlwaysInViewport && by1 == by2) by1 = Math.Max(by1 - 1, 0);
        ...
        {
            if (region.Bottom < Bounds.Height - 1) return;
            ...
            else
            {
                ...
                var bx2 = KeepContentAlwaysInViewport
                    ? Math.Min((position + bw) * bw / Size + 1, bw - 1)
                    : (position + bw) * bw / (Size + bw);
                if (KeepContentAlwaysInViewport && bx1 == bx2) bx1 = Math.Max(bx1 - 1, 0);
    ...
    public override bool MouseEvent(MouseEvent me)
    {
        ...
        if (!Host.CanFocus) return true;
        if (Host?.HasFocus == false) Host.SetFocus();

        var location = vertical ? me.Y : me.X;
        var barsize = vertical ? Bounds.Height : Bounds.Width;
        var posTopLeftTee = vertical ? posTopTee + 1 : posLeftTee + 1;
        var posBottomRightTee = vertical ? posBottomTee + 1 : posRightTee + 1;
        barsize -= 2;
        var pos = Position;
        ...
        else if (me.Flags == MouseFlags.Button1Released && Application.mouseGrabView != null &&
        ...
        {
            if (CanScroll(1, out _, vertical)) Position = pos + 1;
        }
        else if (location > 0 && location < barsize + 1)
        {
                                                                   ...
                                                                   MouseFlags.ReportMousePosition))
            {
                ...
                if (location > lastLocation)
                {
                    if (location - posBarOffset < barsize)
                    {
                        ...
                        if (CanScroll(np - pos, out var nv, vertical)) Position = pos + nv;
                    }
                    else if (CanScroll(Size - pos, out var nv, vertical))
                    {
                ...
                else if (location - posBarOffset >= barsize && posBottomRightTee - posTopLeftTee >= 3 &&
                         CanScroll(Size - pos, out var nv, vertical))
                {
                    Position = Math.Min(pos + nv, Size);
                }
                else if (location - posBarOffset >= barsize - 1 && posBottomRightTee - posTopLeftTee <= 3 &&
                         CanScroll(Size - pos, out nv, vertical))
                {
                    Position = Math.Min(pos + nv, Size);
                }
            ...
            {
                if (CanScroll(barsize, out var nv, vertical)) Position = pos + nv;
            }
            ...
            {
                if (CanScroll(Size - pos, out var nv, vertical)) Position = Math.Min(pos + nv, Size);
            }
        }
    ...
    internal bool CanScroll(int n, out int max, bool isVertical = false)
    {
        ...
        if (size >= s + newSize && max != 0) return true;
        return false;
    }
    ...
    private int GetBarsize(bool isVertical)
    {
        ...
        return isVertical ? KeepContentAlwaysInViewport ? Host.Bounds.Height + (showBothScrollIndicator ? -2 : -1) :
                                                          0 :

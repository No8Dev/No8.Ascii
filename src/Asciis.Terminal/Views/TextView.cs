...
{
    private List<List<Rune>> lines = new();
    ...
    internal static List<Rune> ToRunes(string str) { return str.ToRuneList(); }

    // Splits a string into a List that contains a List<Rune> for each line
    public static List<List<Rune>> StringToRunes(string contents)
    {
        var content = contents.ToRuneArray();
        ...
        for (; i < content.Length; i++)
        {
            ...
            {
                if (i - start > 0)
                    lines.Add(content.Substring(start, (hasCR ? i - 1 : i) - start).ToList());
                else
        ...
        if (i - start >= 0)
            lines.Add(content.Substring(start).ToList());
        return lines;
    }
    ...
    public void LoadString(string content) { lines = StringToRunes(content); }
    ...
    public override string ToString()
    {
        ...
        {
            sb.Append(lines[i].MakeString());
            ;
            if (i + 1 < lines.Count) sb.AppendLine();
        }
    ...
    public void AddLine(int pos, List<Rune> runes) { lines.Insert(pos, runes); }
    ...
    internal static int CalculateLeftColumn(List<Rune> t, int start, int end, int width, int tabWidth = 0)
    {
        ...
        for (var i = tcount; i >= 0; i--)
        {
            var rune = t[i];
            size += rune.ColumnWidth();
            if (rune.Value == '\t') size += tabWidth + 1;
            if (size >= width)
                break;
            else if (end < t.Count && col > 0 && start < end && col == start) break;
            col = i;
        }
    ...
    private (Point startPointToFind, Point currentPointToFind, bool found) toFind;
        ...
        bool matchWholeWord = false)
    {
        ...
        if (toFind.found) toFind.currentPointToFind.X++;
        var foundPos = GetFoundNextTextPoint(
            text,
            lines.Count,
            matchCase,
            matchWholeWord,
            toFind.currentPointToFind);
        if (!foundPos.found && toFind.currentPointToFind != toFind.startPointToFind)
            foundPos = GetFoundNextTextPoint(
                text,
                toFind.startPointToFind.Y + 1,
                matchCase,
                matchWholeWord,
                Point.Empty);
        ...
        bool matchWholeWord = false)
    {
        ...
        var foundPos = GetFoundPreviousTextPoint(
            text,
            linesCount,
            matchCase,
            matchWholeWord,
        ...
        string textToReplace = null)
    {
        ...
        {
            var txt = x.MakeString();
            if (!matchCase) txt = txt.ToUpper();
            return txt;
        }
    ...
    private string ReplaceText(List<Rune> source, string textToReplace, string matchText, int col)
    {
        var origTxt = source.MakeString();
        var (_, len) = DisplaySize(source, 0, col, false);
        var (_, len2) = DisplaySize(source, col, col + matchText.Length, false);
        var (_, len3) = DisplaySize(source, col + matchText.Length, origTxt.RuneCount(), false);

        return origTxt.Substring(0, len) +
    ...
    private bool ApplyToFind((Point current, bool found) foundPos)
    {
        ...
        if (foundPos.found)
        {
            ...
            if (toFind.found && toFind.currentPointToFind == toFind.startPointToFind) gaveFullTurn = true;
            if (!toFind.found)
            {
        ...
        Point start)
    {
        for (var i = start.Y; i < linesCount; i++)
        {
            ...
            if (col > -1 && matchWholeWord && !MatchWholeWord(txt, matchText, col)) continue;
            if (col > -1 && (i == start.Y && col >= start.X
                             || i > start.Y)
                         && txt.Contains(matchText))
                return (new Point(col, i), true);
            else if (col == -1 && start.X > 0) start.X = 0;
        }
        ...
        Point start)
    {
        for (var i = linesCount; i >= 0; i--)
        {
            ...
            var txt = x.MakeString();
            if (!matchCase) txt = txt.ToUpper();
            if (start.Y != i) start.X = Math.Max(x.Count - 1, 0);
            var matchText = !matchCase ? text.ToUpper().ToString() : text.ToString();
    ...
    {
        if (string.IsNullOrEmpty(source) || string.IsNullOrEmpty(matchText)) return false;
...
internal class WordWrapManager
{
    ...
    private List<WrappedLine> wrappedModelLines = new();
    private int frameWidth;
    private bool isWrapModelRefreshing;
    ...
    public List<List<Rune>> ToListRune(List<string> textList)
    {
        ...
        foreach (var text in textList) runesList.Add(text.ToRuneList());
    ...
    {
        return wrappedModelLines.Count > 0
    ...
    private List<Rune> GetCurrentLine(int row) { return Model.GetLine(row); }
    ...
    public bool RemoveLine(int row, int col, out bool lineRemoved, bool forward = true)
    {
        ...
        else
        {
            ...
            Model.RemoveLine(modelRow);
            if (prevLine.Count > frameWidth) return true;
        }
...
public class TextView : View
{
    ...
    private WordWrapManager wrapManager;
    private bool continuousFind;
    private int bottomOffset, rightOffset;
    private int tabWidth = 4;
    private bool allowsTab = true;
    private bool allowsReturn = true;
    ...
    /// </summary>
    public Autocomplete Autocomplete { get; protected set; } = new();
    ...
    /// <remarks>
    /// </remarks>
    public TextView(Rectangle frame)
        : base(frame)
    {
        Initialize();
    }
    ...
    {
        get => topRow;
        set => topRow = Math.Max(Math.Min(value, Lines - 1), 0);
    }
    ...
    {
        get => leftColumn;
        set => leftColumn = Math.Max(Math.Min(value, Maxlength - 1), 0);
    }
    ...
    public Point CursorPosition
    {
        ...
        set
        {
            ...
            currentRow = value.Y < 0
                ? 0
                : value.Y > model.Count - 1
                    ? Math.Max(model.Count - 1, 0)
                    : value.Y;
            SetNeedsDisplay();
            Adjust();
        }
    ...
    public string SelectedText
    {
        get
        {
            if (!selecting || model.Count == 1 && model.GetLine(0).Count == 0) return string.Empty;
    ...
    public bool AllowsReturn
    {
        ...
        {
            allowsReturn = value;
            if (allowsReturn && !multiline) Multiline = true;
    ...
    public bool AllowsTab
    {
        ...
        {
            allowsTab = value;
            if (allowsTab && tabWidth == 0) tabWidth = 4;
            if (allowsTab && !multiline) Multiline = true;
            if (!allowsTab && tabWidth > 0) tabWidth = 0;
            SetNeedsDisplay();
        }
    }
    ...
    public int TabWidth
    {
        ...
        set
        {
            ...
            if (tabWidth > 0 && !AllowsTab) AllowsTab = true;
            SetNeedsDisplay();
        }
    }

    private Dim savedHeight = null;
    ...
    public bool Multiline
    {
        ...
        {
            multiline = value;
            if (multiline && !allowsTab) AllowsTab = true;
            if (multiline && !allowsReturn) AllowsReturn = true;
            ...
            if (!multiline)
            {
                ...
                if (LayoutStyle == LayoutStyle.Computed) LayoutStyle = LayoutStyle.Absolute;
                Height = 1;
                LayoutStyle = lyout;
                SetNeedsDisplay();
            }
    ...
    private void SaveCursorVisibility()
    {
        ...
        {
            if (savedCursorVisibility == 0) savedCursorVisibility = desiredCursorVisibility;
    ...
    public override void PositionCursor()
    {
        ...
        if (selecting)
        {
            ...
            SetNeedsDisplay(new Rectangle(0, minRow, Frame.Width, maxRow));
        }

        var line = model.GetLine(currentRow);
        var col = 0;
        if (line.Count > 0)
            for (var idx = leftColumn; idx < line.Count; idx++)
            {
                if (idx >= currentColumn)
                    break;
                var cols = line[idx].ColumnWidth();
                if (line[idx].Value == '\t') cols += TabWidth + 1;
    ...
    /// current <paramref name="line"/>.  Override to provide custom coloring by calling <see cref="ConsoleDriver.SetAttribute"/>
    /// Defaults to <see cref="ColorScheme.Normal"/>.
    /// </summary>
    /// <param name="line"></param>
    /// <param name="idx"></param>
    protected virtual void ColorNormal(List<Rune> line, int idx) { Driver.SetAttribute(GetNormalColor()); }
    ...
    /// <param name="line"></param>
    /// <param name="idx"></param>
    protected virtual void ColorSelection(List<Rune> line, int idx) { Driver.SetAttribute(ColorScheme.Focus); }
    ...
    /// <param name="line"></param>
    /// <param name="idx"></param>
    protected virtual void ColorUsed(List<Rune> line, int idx) { Driver.SetAttribute(ColorScheme.HotFocus); }
    ...
    private string GetRegion()
    {
        ...
        for (var row = startRow + 1; row < maxrow; row++) res = res + "\n" + StringFromRunes(model.GetLine(row));
        line = model.GetLine(maxrow);
        res = res + "\n" + StringFromRunes(line.GetRange(0, endCol));
        return res;
    }
    ...
    private void ClearRegion()
    {
        ...
        if (startRow == maxrow)
        {
            ...
            if (wordWrap)
                ...
                SetNeedsDisplay(new Rectangle(0, startRow - topRow, Frame.Width, startRow - topRow + 1));
            return;
        }
    ...
    public bool ReplaceAllText(
        ...
        string textToReplace = null)
    {
        if (isReadOnly || model.Count == 0) return false;
    ...
    /// </summary>
    private void UpdateWrapModel()
    {
        if (wordWrap)
        {
            wrapManager.UpdateModel(
                model,
                out var nRow,
                out var nCol,
                out var nStartRow,
    ...
    public override void Redraw(Rectangle bounds)
    {
        ...
        var row = 0;
        for (var idxRow = topRow; idxRow < model.Count; idxRow++)
        {
            ...
            Move(0, row);
            for (var idxCol = leftColumn; idxCol < lineRuneCount; idxCol++)
            {
                var rune = idxCol >= lineRuneCount ? new Rune(' ') : line[idxCol];
                ...
                if (!TextModel.SetCol(ref col, bounds.Right, cols)) break;
                if (idxCol + 1 < lineRuneCount && col + line[idxCol + 1].ColumnWidth() > right) break;
            }
    ...
    {
        if (text != null) Application.Driver.Clipboard.Contents = text;
    }

    private void SetClipboard(Rune rune) { Application.Driver.Clipboard.Contents = rune.ToString(); }

    private void AppendClipboard(string text) { Application.Driver.Clipboard.Contents += text; }

    private void AppendClipboard(Rune rune) { Application.Driver.Clipboard.Contents += rune.ToString(); }
    ...
    private void Insert(uint ch) { Insert((Rune)ch); }
    ...
    private void Insert(Rune rune)
    {
        ...
        {
            if (currentColumn < line.Count) line.RemoveAt(currentColumn);
            line.Insert(Math.Min(currentColumn, line.Count), rune);
        }
        ...
        if (wordWrap)
        {
            ...
            if (wrapNeeded) SetNeedsDisplay();
        }

        var prow = currentRow - topRow;
        if (!wrapNeeded) SetNeedsDisplay(new Rectangle(0, prow, Frame.Width, prow + 1));
    }

    private string StringFromRunes(List<Rune> runes)
    {
        if (runes == null)
            throw new ArgumentNullException(nameof(runes));
        var size = 0;
        foreach (var rune in runes) size += rune.Utf8SequenceLength;
        var encoded = new byte[size];
        var offset = 0;
        foreach (var rune in runes) offset += rune.EncodeToUtf8(encoded.AsSpan(offset));
        //return encoded;
    ...
    /// </summary>
    /// <returns></returns>
    public List<Rune> GetCurrentLine() { return model.GetLine(currentRow); }
    ...
    */
    ...
    private void Adjust()
    {
        var offB = OffSetBackground();
        var line = GetCurrentLine();
        var need = !NeedDisplay.IsEmpty || wrapNeeded;
        ...
        {
            leftColumn = TextModel.CalculateLeftColumn(
                line,
                leftColumn,
                currentColumn,
    ...
    private (int width, int height) OffSetBackground()
    {
        var w = 0;
        var h = 0;
        if (SuperView?.Frame.Right - Frame.Right < 0) w = SuperView.Frame.Right - Frame.Right - 1;
        if (SuperView?.Frame.Bottom - Frame.Bottom < 0) h = SuperView.Frame.Bottom - Frame.Bottom - 1;
        return (w, h);
    }
    ...
    public override bool ProcessKey(KeyEvent kb)
    {
        ...
        if (kb.Key == Key.CursorRight)
        {
            ...
            if (currentColumn == lastCol && currentRow == lastRow) return false;
        }

        // Give autocomplete first opportunity to respond to key presses
        if (Autocomplete.ProcessKey(this, kb)) return true;
        ...
        switch (kb.Key)
        {
            ...
            case Key.PageDown | Key.ShiftMask:
                if (kb.Key.HasFlag(Key.ShiftMask))
                    StartSelecting();
                else if (shiftSelecting && selecting) StopSelecting();
            ...
            case Key.Backspace:
                ...
                if (DeleteTextBackwards()) return true;
                break;
            ...
            case Key.D | Key.CtrlMask: // Delete
                ...
                if (DeleteTextForwards()) return true;
                break;
            ...
            case Key.DeleteChar | Key.CtrlMask | Key.ShiftMask:
                ...
                {
                    if (currentRow < model.Count - 1) model.RemoveLine(currentRow);
                ...
                SetNeedsDisplay(new Rectangle(0, currentRow - topRow, Frame.Width, Frame.Height));
            ...
            case Key.Backspace | Key.CtrlMask | Key.ShiftMask: // kill-to-start
                ...
                else
                {
                    ...
                    var val = string.Empty;
                    if (currentColumn == 0 && lastWasKill && currentLine.Count > 0) val = "\n";
                    val += StringFromRunes(rest);
                    if (lastWasKill)
                        AppendClipboard(val);
                    else
            ...
            case Key.DeleteChar | Key.CtrlMask: // kill-word-forwards
                ...
                if (wordWrap && wrapManager.RemoveRange(currentRow, currentColumn, restCount)) wrapNeeded = true;
                if (wrapNeeded)
                    SetNeedsDisplay();
                else
                    SetNeedsDisplay(new Rectangle(0, currentRow - topRow, Frame.Width, Frame.Height));
            ...
            case Key.Backspace | Key.CtrlMask: // kill-word-backwards
                ...
                if (newPos.HasValue && currentRow == newPos.Value.row)
                {
                    ...
                    if (wordWrap && wrapManager.RemoveRange(currentRow, newPos.Value.col, restCount)) wrapNeeded = true;
                    currentColumn = newPos.Value.col;
                }
                else if (newPos.HasValue)
                {
            ...
            case Key.Enter:
                ...
                if (fullNeedsDisplay)
                    ...
                    SetNeedsDisplay(new Rectangle(0, currentRow - topRow, 2, Frame.Height));
                break;
            ...
            case Key _ when ShortcutHelper.GetModifiersKey(kb) == Key.Tab:
                ...
                InsertText(new KeyEvent((Key)'\t'));
                break;

            case Key _ when ShortcutHelper.GetModifiersKey(kb) == (Key.BackTab | Key.ShiftMask):
                if (!AllowsTab) return false;
                if (currentColumn > 0)
                {
                    currentLine = GetCurrentLine();
                    if (currentLine.Count > 0 && currentLine[currentColumn - 1].Value == '\t')
                    {
    ...
    private bool InsertText(KeyEvent kb)
    {
        //So that special keys like tab can be processed
        if (isReadOnly)
            return true;
        if (selecting) ClearSelectedRegion();
        if (Used)
        {
            Insert((uint)kb.Key);
            currentColumn++;
    ...
    private bool DeleteTextForwards()
    {
        ...
        if (currentColumn == currentLine.Count)
        {
            ...
            if (wordWrap && wrapManager.RemoveLine(currentRow, currentColumn, out _)) wrapNeeded = true;
            var sr = currentRow - topRow;
            SetNeedsDisplay(new Rectangle(0, sr, Frame.Width, sr + 1));
        }
        else
        {
            currentLine.RemoveAt(currentColumn);
            if (wordWrap && wrapManager.RemoveAt(currentRow, currentColumn)) wrapNeeded = true;
            var r = currentRow - topRow;
            SetNeedsDisplay(new Rectangle(currentColumn - leftColumn, r, Frame.Width, r + 1));
        }
    ...
    public bool DeleteTextBackwards()
    {
        if (currentColumn > 0)
        {
            ...
            var currentLine = GetCurrentLine();
            currentLine.RemoveAt(currentColumn - 1);
            if (wordWrap && wrapManager.RemoveAt(currentRow, currentColumn - 1)) wrapNeeded = true;
            currentColumn--;
            ...
            {
                SetNeedsDisplay(new Rectangle(0, currentRow - topRow, 1, Frame.Width));
            }
        }
        else
        {
    ...
    private bool copyWithoutSelection;
    ...
    public void Copy()
    {
        ...
        {
            var currentLine = GetCurrentLine();
            SetClipboard(currentLine.MakeString());
    ...
    public void Paste()
    {
        ...
        SetWrapModel();
        var contents = Application.Driver.Clipboard.Contents;
        if (copyWithoutSelection)
        {
            var runeList = contents == null ? new List<Rune>() : contents.ToRuneList();
            if (runeList?.Count > 0 && runeList[0].Value == '\n'
                || runeList?.Count > 1 && runeList[0].Value == '\r'
                                       && runeList[1].Value == '\n')
            {
                InsertText(contents);
            }
    ...
    {
        if (shiftSelecting && selecting) return;
        shiftSelecting = true;
        selecting = true;
        selectionStartColumn = currentColumn;
        selectionStartRow = currentRow;
    ...
    private void MoveUp()
    {
        if (currentRow > 0)
        {
            if (columnTrack == -1) columnTrack = currentColumn;
            currentRow--;
            if (currentRow < topRow)
            {
                topRow--;
    ...
    private (int col, int row)? WordForward(int fromCol, int fromRow)
    {
        ...
        try
        {
            ...
            void ProcMoveNext(ref int nCol, ref int nRow, Rune nRune)
            {
                if (Rune.IsSymbol(nRune) || Rune.IsWhiteSpace(nRune))
                {
                    while (MoveNext(ref nCol, ref nRow, out nRune))
                        if (Rune.IsLetterOrDigit(nRune) || Rune.IsPunctuation(nRune))
                            return;
                    if (nRow != fromRow && (Rune.IsLetterOrDigit(nRune) || Rune.IsPunctuation(nRune))) return;
                    while (MoveNext(ref nCol, ref nRow, out nRune))
                        if (!Rune.IsLetterOrDigit(nRune) && !Rune.IsPunctuation(nRune))
                            break;
                }
                ...
                {
                    if (!MoveNext(ref nCol, ref nRow, out nRune)) return;
                    ...
                    else if (nRow != fromRow && fromCol == line.Count)
                    {
                        ...
                        if (Rune.IsLetterOrDigit(line[nCol]) || Rune.IsPunctuation(line[nCol])) return;
                    }
    ...
    private (int col, int row)? WordBackward(int fromCol, int fromRow)
    {
        ...
        {
            var rune = RuneAt(col, row);
            var lastValidCol = Rune.IsLetterOrDigit(rune) || Rune.IsPunctuation(rune) ? col : -1;
            ...
            void ProcMovePrev(ref int nCol, ref int nRow, Rune nRune)
            {
                if (Rune.IsSymbol(nRune) || Rune.IsWhiteSpace(nRune))
                {
                    ...
                    {
                        if (lastValidCol > -1) nCol = lastValidCol;
                        return;
                    }
                ...
                {
                    if (!MovePrev(ref nCol, ref nRow, out nRune)) return;

                    var line = model.GetLine(nRow);
                    if (nCol == 0 && nRow == fromRow &&
                        (Rune.IsLetterOrDigit(line[0]) || Rune.IsPunctuation(line[0]))) return;
    ...
    public override bool MouseEvent(MouseEvent ev)
    {
        if (!ev.Flags.HasFlag(MouseFlags.Button1Clicked) && !ev.Flags.HasFlag(MouseFlags.Button1Pressed)
                                                         ...
                                                         && !ev.Flags.HasFlag(
                                                             MouseFlags.Button1Pressed | MouseFlags.ButtonShift)
        ...
        else if (ev.Flags.HasFlag(MouseFlags.Button1Pressed | MouseFlags.ReportMousePosition))
        {
            ...
            if (model.Count > 0 && shiftSelecting && selecting)
            {
                ...
                else if (ev.Y < 0 && topRow > 0) ScrollTo(0);
                ...
                else if (ev.X < 0 && leftColumn > 0) ScrollTo(0, false);
            }
        ...
        {
            if (!shiftSelecting) StartSelecting();
            ProcessMouseClick(ev, out _);
            PositionCursor();
            lastWasKill = false;
            columnTrack = currentColumn;
        ...
        else if (ev.Flags.HasFlag(MouseFlags.Button1DoubleClicked))
        {
            ...
            if (currentColumn > 0 && line[currentColumn - 1].Value != ' ')
            {
                newPos = WordBackward(currentColumn, currentRow);
                if (newPos.HasValue)
                {
            ...
            if (currentRow < selectionStartRow ||
                currentRow == selectionStartRow && currentColumn < selectionStartColumn)
            {
                if (currentColumn > 0 && line[currentColumn - 1].Value != ' ')
                    newPos = WordBackward(currentColumn, currentRow);
            }
    ...
    private void ProcessMouseClick(MouseEvent ev, out List<Rune> line)
    {
        List<Rune> r = null;
        if (model.Count > 0)
        {
    ...
    {
        if (Application.mouseGrabView != null && Application.mouseGrabView == this) Application.UngrabMouse();

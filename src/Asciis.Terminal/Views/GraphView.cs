...
using Asciis.Terminal.Core.Graphs;
...
public class GraphView : View
{
    ...
    /// </summary>
    public CellAttributes? GraphColor { get; set; }
    ...
    {
        if (CellSize.X == 0 || CellSize.Y == 0) throw new Exception($"{nameof(CellSize)} cannot be 0");
        ...
        if (!Series.Any() && !Annotations.Any()) return;

        // Draw 'before' annotations
        foreach (var a in Annotations.ToArray().Where(a => a.BeforeSeries)) a.Render(this);
        ...
        var drawBounds = new Rectangle(
            (int)MarginLeft,
            0,
            Bounds.Width - (int)MarginLeft,
            Bounds.Height - (int)MarginBottom);
    ...
    private int ipart(decimal x) { return (int)x; }


    private decimal fpart(decimal x)
    {

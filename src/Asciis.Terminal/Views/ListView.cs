...
public class ListView : View
{
    ...
    public ListView(Rectangle rect, IList source)
        : this(rect, MakeWrapper(source))
    {
        Initialize();
    }
    ...
    public ListView(Rectangle rect, IListDataSource source)
        : base(rect)
    {
        this.source = source;
        Initialize();
    ...
    public override void Redraw(Rectangle bounds)
    {
        ...
        for (var row = 0; row < f.Height; row++, item++)
        {
            ...
            else
            {
                ...
                {
                    Driver.AddRune(
                        source.IsMarked(item) ? AllowsMultipleSelection ? Driver.Checked : Driver.Selected :
    ...
    public virtual bool MoveUp()
    {
        ...
        else if (selected > 0)
        {
            ...
            if (selected < top)
                top = selected;
            else if (selected > top + Frame.Height) top = Math.Max(selected - Frame.Height + 1, 0);
            OnSelectedChanged();
            SetNeedsDisplay();
        }
    ...
    public virtual bool MoveEnd()
    {
        ...
        {
            selected = source.Count - 1;
            if (top + selected > Frame.Height - 1) top = selected;
            OnSelectedChanged();
            SetNeedsDisplay();
        }
    ...
    public virtual bool OnSelectedChanged()
    {
        if (selected != lastSelectedItem)
        {
            ...
            if (HasFocus) lastSelectedItem = selected;
            return true;
        }
    ...
    {
        if (source.Count <= selected || selected < 0 || OpenSelectedItem == null) return false;
    ...
    private void EnsuresVisibilitySelectedItem()
    {
        SuperView?.LayoutSubviews();
        if (selected < top)
            top = selected;
        else if (Frame.Height > 0 && selected >= top + Frame.Height) top = Math.Max(selected - Frame.Height + 2, 0);
    }
...
/// <summary>
/// <see cref="System.EventArgs"/> for <see cref="ListView"/> events.
/// </summary>
public class ListViewItemEventArgs : System.EventArgs
{
    /// <summary>

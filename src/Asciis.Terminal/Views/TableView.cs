...
public class TableView : View
{
    ...
    /// </summary>
    public class CellActivatedEventArgs : System.EventArgs
    {
        /// <summary>
        ...
        set
        {
            ...
            if (oldValue != selectedColumn)
                OnSelectedCellChanged(
                    new SelectedCellChangedEventArgs(Table, oldValue, SelectedColumn, SelectedRow, SelectedRow));
        }
    }
    ...
    public int SelectedRow
    {
        ...
        set
        {
            ...
            if (oldValue != selectedRow)
                OnSelectedCellChanged(
                    new SelectedCellChangedEventArgs(Table, SelectedColumn, SelectedColumn, oldValue, selectedRow));
        }
    }
    ...
    /// </summary>
    /// <returns></returns>
    private int GetHeaderHeightIfAny() { return ShouldRenderHeaders() ? GetHeaderHeight() : 0; }
    ...
    private void RenderHeaderUnderline(int row, int availableWidth, ColumnToRender[] columnsToRender)
    {
        ...
        for (var c = 0; c < availableWidth; c++)
        {
            ...
            if (Style.ShowVerticalHeaderLines)
            {
                ...
                // if the next column is the start of a header
                else if (columnsToRender.Any(r => r.X == c + 1)) /*TODO: is ┼ symbol in Driver?*/
                    rune = Style.ShowVerticalCellLines ? (Rune)'┼' : Driver.BottomTee;
                else if (c == availableWidth - 1)
    ...
    private void RenderRow(int row, int rowToRender, ColumnToRender[] columnsToRender)
    {
        ...
        Move(0, row);
        Driver.SetAttribute(
            FullRowSelect && IsSelected(0, rowToRender) ? rowScheme.HotFocus
        ...
        for (var i = 0; i < columnsToRender.Length; i++)
        {
            ...
            var isSelectedCell = IsSelected(current.Column.Ordinal, rowToRender);
            ...
            if (colorSchemeGetter != null)
            {
                ...
                if (scheme == null) scheme = rowScheme;
            }
            else
            {
            ...
            var isPrimaryCell = current.Column.Ordinal == selectedColumn && rowToRender == selectedRow;
            ...
            if (scheme != rowScheme)
                Driver.SetAttribute(
                    isSelectedCell ? rowScheme.HotFocus
                    : Enabled ? rowScheme.Normal : rowScheme.Disabled);
    ...
    protected virtual void RenderCell(CellAttributes cellColor, string render, bool isPrimaryCell)
    {
        ...
        if (Style.InvertSelectedCellFirstCharacter && isPrimaryCell)
        {
            if (render.Length > 0)
            {
                ...
                Driver.SetAttribute(Driver.MakeCellColor(cellColor.Background, cellColor.Foreground));
    ...
    private void RenderSeparator(int col, int row, bool isHeader)
    {
        ...
        var symbol = renderLines ? Driver.VLine : SeparatorSymbol;
        ...
        ColumnStyle colStyle)
    {
        ...
        if (representation.EnumerateRunes().Sum(c => c.ColumnWidth()) < availableHorizontalSpace)
        {
            ...
            var toPad = availableHorizontalSpace -
                        (representation.EnumerateRunes().Sum(c => c.ColumnWidth()) +
        ...
        return representation
              .EnumerateRunes()
              .TakeWhile(c => (availableHorizontalSpace -= c.ColumnWidth()) > 0)
              .ToArray()
              .MakeString();
    }
        ...
        switch (keyEvent.Key)
        {
            ...
            case Key.PageUp | Key.ShiftMask:
                ChangeSelectionByOffset(
                    0,
                    -(Bounds.Height - GetHeaderHeightIfAny()),
                    keyEvent.Key.HasFlag(Key.ShiftMask));
                Update();
                break;
            case Key.PageDown:
    ...
    public void SelectAll()
    {
        ...
        MultiSelectedRegions.Push(
            new TableSelection(
                ...
                new Rectangle(0, 0, Table.Columns.Count, table.Rows.Count)));
        Update();
    }
    ...
    public IEnumerable<Point> GetAllSelectedCells()
    {
        ...
        else
        {
            ...
            if (FullRowSelect) // all cells in active row are selected
                for (var x = 0; x < Table.Columns.Count; x++)
                    yield return new Point(x, SelectedRow);
            else // Not full row select and no multi selections
    ...
    public override bool MouseEvent(MouseEvent me)
    {
        ...
        {
            var hit = ScreenToCell(me.X, me.Y);
            if (hit != null) OnCellActivated(new CellActivatedEventArgs(Table, hit.Value.X, hit.Value.Y));
        }
    ...
    public Point? ScreenToCell(int clientX, int clientY)
    {
        ...
        if (col != null && rowIdx >= 0) return new Point(col.Column.Ordinal, rowIdx);
    ...
    public void EnsureValidSelection()
    {
        ...
        foreach (var region in oldRegions)
        {
            ...
            region.Rect = Rectangle.FromLTRB(
                region.Rect.Left,
                region.Rect.Top,
    ...
    public void EnsureSelectedCellIsVisible()
    {
        ...
        if (SelectedColumn < columnsToRender.Min(r => r.Column.Ordinal)) ColumnOffset = SelectedColumn;

        //if we have scrolled too far to the right
        if (SelectedColumn > columnsToRender.Max(r => r.Column.Ordinal)) ColumnOffset = SelectedColumn;

        //if we have scrolled too far down
        if (SelectedRow >= RowOffset + (Bounds.Height - headerHeight)) RowOffset = SelectedRow;
    ...
    protected virtual void OnCellActivated(CellActivatedEventArgs args) { CellActivated?.Invoke(args); }
    ...
    private IEnumerable<ColumnToRender> CalculateViewport(Rectangle bounds, int padding = 1)
    {
        ...
        var availableHorizontalSpace = bounds.Width;
        ...
        {
            var startingIdxForCurrentHeader = usedSpace;
            ...
            // there is space
            yield return new ColumnToRender(
                col,
                startingIdxForCurrentHeader,
    ...
    {
        var spaceRequired = col.ColumnName.EnumerateRunes().Sum(c => c.ColumnWidth());
        ...
        for (var i = RowOffset;
            i < RowOffset + rowsToRender && i < Table.Rows.Count;
            i++) //expand required space if cell is bigger than the last biggest cell or header
            spaceRequired = Math.Max(
                spaceRequired,
                GetRepresentation(Table.Rows[i][col], colStyle).EnumerateRunes().Sum(c => c.ColumnWidth()));

        // Don't require more space than the style allows
        if (colStyle != null)
        {
            // enforce maximum cell width based on style
            if (spaceRequired > colStyle.MaxWidth) spaceRequired = colStyle.MaxWidth;

            // enforce minimum cell width based on style
            if (spaceRequired < colStyle.MinWidth) spaceRequired = colStyle.MinWidth;
        }
    ...
    {
        if (value == null || value == DBNull.Value) return NullSymbol;
    ...
    public class ColumnStyle
    {
        ...
        /// </summary>
        public int MaxWidth { get; set; } = DefaultMaxCellWidth;
    ...
    public class TableStyle
    {
        ...
        /// </summary>
        public Dictionary<DataColumn, ColumnStyle> ColumnStyles { get; set; } = new();
    ...
    internal class ColumnToRender
    {
        ...
        /// <summary>
        /// The width that the column should occupy as calculated by <see cref="TableView.CalculateViewport(Rectangle, int)"/>.  Note that this includes
    ...
    /// </summary>
    public class SelectedCellChangedEventArgs : System.EventArgs
    {
        /// <summary>
        /// The current table to which the new indexes refer.  May be null e.g. if selection change is the result of clearing the table from the view
        /// </summary>

...
/// <summary>
/// A status bar is a <see cref="View"/> that snaps to the bottom of a <see cref="Window"/> displaying set of <see cref="StatusItem"/>s.
...
public class StatusBar : View
{
    ...
    private void StatusBar_Initialized(object sender, System.EventArgs e)
    {
        ...
        if (SuperView.Frame == Rectangle.Empty)
            ((Window)SuperView).Loaded += StatusBar_Loaded;
        else
            Y = Math.Max(SuperView.Frame.Height - (base.Visible ? 1 : 0), 0);
    }
    ...
    private EventHandler<ResizedEventArgs> Application_Resized()
    {
        return delegate
        {
            X = 0;
            Height = 1;
            if (SuperView is Window win)
            {
                if (Frame.Y != win.Frame.Height - (Visible ? 1 : 0))
                    Y = win.Frame.Height - (Visible ? 1 : 0);
            }
        };
    }
    ...
    private CellAttributes ToggleScheme(CellAttributes scheme)
    {

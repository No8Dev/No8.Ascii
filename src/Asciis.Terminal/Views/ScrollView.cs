...
/// <remarks>
/// <para>
///   The subviews that are added to this <see cref="ScrollView"/> are offset by the
...
public class ScrollView : View
{
    ...
    private ScrollBarView vertical, horizontal;

    /// <summary>
    ///  Initializes a new instance of the <see cref="ScrollView"/> class using <see cref="LayoutStyle.Absolute"/> positioning.
    /// </summary>
    /// <param name="frame"></param>
    public ScrollView(Rectangle frame)
        : base(frame)
    {
        Init(frame);
    }
    ...
    /// <summary>
    ///  Initializes a new instance of the <see cref="ScrollView"/> class using <see cref="LayoutStyle.Computed"/> positioning.
    /// </summary>
    public ScrollView()
        : base()
    {
        Init(new Rectangle(0, 0, 0, 0));
    }

    private void Init(Rectangle frame)
    {
        contentView = new View(frame);
        vertical = new ScrollBarView(1, 0, true)
        {
            X = Pos.AnchorEnd(1),
            Y = 0,
            Width = 1,
        ...
        vertical.ChangedPosition += delegate { ContentOffset = new Point(ContentOffset.X, vertical.Position); };
        vertical.Host = this;
        horizontal = new ScrollBarView(1, 0, false)
        {
            X = 0,
            Y = Pos.AnchorEnd(1),
        ...
        horizontal.ChangedPosition += delegate { ContentOffset = new Point(horizontal.Position, ContentOffset.Y); };
    ...
    public Size ContentSize
    {
        ...
        set
        {
            ...
            {
                contentSize = value;
                contentView.Frame = new Rectangle(contentOffset, value);
    ...
    public Point ContentOffset
    {
        ...
        set
        {
            ...
            {
                contentOffset = co;
                contentView.Frame = new Rectangle(contentOffset, contentSize);
                var p = Math.Max(0, -contentOffset.Y);
                if (vertical.Position != p) vertical.Position = Math.Max(0, -contentOffset.Y);
                p = Math.Max(0, -contentOffset.X);
                if (horizontal.Position != p) horizontal.Position = Math.Max(0, -contentOffset.X);
                SetNeedsDisplay();
            }
        }
    }
        ...
        set
        {
            if (keepContentAlwaysInViewport != value)
            {
                ...
                if (value && -contentOffset.X + Bounds.Width > contentSize.Width)
                    p = new Point(
                        contentSize.Width - Bounds.Width + (showVerticalScrollIndicator ? 1 : 0),
                        -contentOffset.Y);
                if (value && -contentOffset.Y + Bounds.Height > contentSize.Height)
                {
                    if (p == default)
                        p = new Point(
                            -contentOffset.X,
                            contentSize.Height - Bounds.Height + (showHorizontalScrollIndicator ? 1 : 0));
                    else
                        p.Y = contentSize.Height - Bounds.Height + (showHorizontalScrollIndicator ? 1 : 0);
                }

                if (p != default) ContentOffset = p;
            }
        }
    }
    ...
    private void View_MouseLeave(MouseEventArgs e)
    {
        if (Application.mouseGrabView != null && Application.mouseGrabView != vertical &&
            Application.mouseGrabView != horizontal) Application.UngrabMouse();
    }

    private void View_MouseEnter(MouseEventArgs e) { Application.GrabMouse(this); }

    private bool IsOverridden(View view)
    {
        Type t = view.GetType();
        MethodInfo m = t.GetMethod("MouseEvent");

        return (m.DeclaringType == t || m.ReflectedType == t) &&
               m.GetBaseDefinition().DeclaringType == typeof(View);
    }
    ...
    public bool ShowHorizontalScrollIndicator
    {
        ...
        set
        {
            if (value == showHorizontalScrollIndicator) return;
    ...
    public bool ShowVerticalScrollIndicator
    {
        ...
        set
        {
            if (value == showVerticalScrollIndicator) return;
    ...
    public override void Redraw(Rectangle region)
    {
        ...
        {
            if (ShowVerticalScrollIndicator) vertical.Redraw(vertical.Bounds);

            if (ShowHorizontalScrollIndicator) horizontal.Redraw(horizontal.Bounds);
        }
    ...
    private void ShowHideScrollBars()
    {
        ...
        {
            if (ShowVerticalScrollIndicator) ShowVerticalScrollIndicator = false;
            v = false;
        }
        ...
        {
            if (!ShowVerticalScrollIndicator) ShowVerticalScrollIndicator = true;
            v = true;
        }

        if (Bounds.Width == 0 || Bounds.Width > contentSize.Width)
        {
            if (ShowHorizontalScrollIndicator) ShowHorizontalScrollIndicator = false;
            h = false;
        }
        ...
        else
        {
            ...
            if (!ShowHorizontalScrollIndicator) ShowHorizontalScrollIndicator = true;
            h = true;
        }

        var dim = Dim.Fill(h ? 1 : 0);
        if (!vertical.Height.Equals(dim)) vertical.Height = dim;
        dim = Dim.Fill(v ? 1 : 0);
        if (!horizontal.Width.Equals(dim)) horizontal.Width = dim;
    ...
    {
        foreach (View view in contentView.Subviews) view.SetNeedsDisplay();
    }
    ...
    {
        if (!showVerticalScrollIndicator) // It was not added to SuperView, so it won't get disposed automatically
            vertical?.Dispose();
        if (!showHorizontalScrollIndicator) // It was not added to SuperView, so it won't get disposed automatically
            horizontal?.Dispose();

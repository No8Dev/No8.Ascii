...
public class TabView : View
{
    ...
    /// </summary>
    /// <value></value>
    public IReadOnlyCollection<Tab> Tabs => tabs.AsReadOnly();
    ...
    public Tab SelectedTab
    {
        ...
        set
        {
            ...
            if (selectedTab != null)
                if (selectedTab.View != null) // remove old content
                    contentView.Remove(selectedTab.View);

            selectedTab = value;

            if (value != null) // add new content
                if (selectedTab.View != null)
                    contentView.Add(selectedTab.View);

            EnsureSelectedTabIsVisible();

            if (old != value) OnSelectedTabChanged(old, value);
        }
    }
    ...
    public override void Redraw(Rectangle bounds)
    {
        ...
        if (Style.ShowBorder)
        {
            ...
            var spaceAtBottom = Math.Max(0, GetTabHeight(false) - 1);
            var startAtY = Math.Max(0, GetTabHeight(true) - 1);
            ...
            DrawFrame(
                new Rectangle(
                    ...
                    startAtY,
                    bounds.Width,
                    bounds.Height - spaceAtBottom - startAtY),
                0,
                true);
        }
    ...
    public void EnsureSelectedTabIsVisible()
    {
        ...
        if (!CalculateViewport(Bounds)
           .Any(r => Equals(SelectedTab, r.Tab))) // Set scroll offset so the first tab rendered is the
    ...
    private IEnumerable<TabToRender> CalculateViewport(Rectangle bounds)
    {
        ...
        foreach (var tab in Tabs.Skip(TabScrollOffset))
        {
            ...
            if (i + tabTextWidth >= bounds.Width) break;
    ...
    private class TabRowView : View
    {
        ...
        public override void Redraw(Rectangle bounds)
        {
            ...
            if (host.Style.ShowTopLine) RenderOverline(tabLocations, width);
        ...
        private void RenderOverline(TabToRender[] tabLocations, int width)
        {
            ...
            var y = host.Style.TabsOnBottom ? 2 : 0;
            ...
            for (var i = 0; i < selected.Width; i++)
            {
                if (selected.X + i > width) // we ran out of space horizontally
                    return;
        ...
        private void RenderTabLine(TabToRender[] tabLocations, int width)
        {
            ...
            foreach (var toRender in tabLocations)
            {
                ...
                {
                    if (host.Focused == this) // if focus is the tab bar ourself then show that they can switch tabs
                        Driver.SetAttribute(ColorScheme.HotFocus);
                    else // Focus is inside the tab
                ...
                if (toRender.IsSelected) Driver.AddRune(Driver.VLine);
            }
        }
        ...
        private void RenderUnderline(TabToRender[] tabLocations, int width)
        {
            ...
            Driver.AddRune(
                selected.X + selected.Width == width - 1 ? Driver.VLine :
        ...
        private int ScreenToScrollIndicator(int x, int y)
        {
            ...
            if (host.TabScrollOffset > 0 && x == 0) return y == GetUnderlineYPosition() ? -1 : 0;

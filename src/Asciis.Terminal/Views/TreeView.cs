...
using Asciis.Terminal.Core.Trees;
...
public class TreeView<T> : View, ITreeView where T : class
{
    ...
    /// </summary>
    private Stack<TreeSelection<T>> multiSelectedRegions = new();
    ...
    public void Remove(T o)
    {
        if (roots.ContainsKey(o))
        {
            ...
            if (Equals(SelectedObject, o)) SelectedObject = default;
        }
    }
    ...
    {
        foreach (var branch in roots.Values) branch.Rebuild();
    ...
    public IEnumerable<T> GetChildren(T o)
    {
        ...
        if (branch == null || !branch.IsExpanded) return new T[0];
    ...
    public int GetContentWidth(bool visible)
    {
        ...
        if (visible)
        {
            ...
            if (ScrollOffsetVertical >= map.Count) return 0;
    ...
    {
        if (cachedLineMap != null) return cachedLineMap;

        List<Branch<T>> toReturn = new();

        foreach (var root in roots.Values) toReturn.AddRange(AddToLineMap(root));
    ...
    private IEnumerable<Branch<T>> AddToLineMap(Branch<T> currentBranch)
    {
        ...
        if (currentBranch.IsExpanded)
            ...
            {
                foreach (var sub in AddToLineMap(subBranch)) yield return sub;
            }
    }
    ...
    public override bool ProcessKey(KeyEvent keyEvent)
    {
        ...
        if (keyEvent.KeyValue > 0 && keyEvent.KeyValue < 0xFFFF)
        {
            ...
            if (char.IsLetterOrDigit(character) && AllowLetterBasedNavigation && !keyEvent.IsShift &&
                !keyEvent.IsAlt && !keyEvent.IsCtrl)
            {
                // search for next branch that begins with that letter
                var characterAsStr = character.ToString();
                AdjustSelectionToNext(
                    b => AspectGetter(b.Model)
                       .StartsWith(characterAsStr, StringComparison.CurrentCultureIgnoreCase));
    ...
    /// </summary>
    /// <param name="e"></param>
    protected virtual void OnObjectActivated(ObjectActivatedEventArgs<T> e) { ObjectActivated?.Invoke(e); }
    ...
    public override bool MouseEvent(MouseEvent me)
    {
        ...
        if (!me.Flags.HasFlag(MouseFlags.Button1Clicked) &&
            ...
            !me.Flags.HasFlag(MouseFlags.WheeledLeft)) // do nothing
            return false;
    ...
    {
        if (ObjectToBranch(toSelect) == null) return;
    ...
    public void AdjustSelection(int offset, bool expandSelection = false)
    {
        ...
        if (!expandSelection || !MultiSelect) multiSelectedRegions.Clear();
    ...
    private void AdjustSelectionToNext(Func<Branch<T>, bool> predicate)
    {
        ...
        if (SelectedObject != null) idxStart = map.IndexOf(b => Equals(b.Model, SelectedObject));
    ...
    public void EnsureVisible(T model)
    {
        ...
        if (idx < ScrollOffsetVertical) //if user has scrolled up too far to see their selection
            ScrollOffsetVertical = idx;
        else if (
            idx >= ScrollOffsetVertical + Bounds.Height -
            leaveSpace) //if user has scrolled off bottom of visible tree
    ...
    {
        foreach (var item in roots) item.Value.ExpandAll();
    ...
    {
        foreach (var item in roots) item.Value.Collapse();
    ...
    protected void CollapseImpl(T toCollapse, bool all)
    {
        ...
        if (SelectedObject != null &&
            ObjectToBranch(SelectedObject) == null) // If the old selection suddenly became invalid then clear it
            SelectedObject = null;
    ...
    /// </summary>
    protected void InvalidateLineMap() { cachedLineMap = null; }
    ...
    public IEnumerable<T> GetAllSelectedObjects()
    {
        ...
        if (MultiSelect)
        {
            foreach (var m in map.Select(b => b.Model).Where(IsSelected)) yield return m;
        }
        else
        {
            if (SelectedObject != null) yield return SelectedObject;
        }
    }
...
internal class TreeSelection<T> where T : class
{
    ...
    private HashSet<T> included = new();
    ...
    public TreeSelection(Branch<T> from, int toIndex, IReadOnlyCollection<Branch<T>> map)
    {
        ...
        foreach (var alsoInclude in map.Skip(lowIndex).Take(highIndex - lowIndex)) included.Add(alsoInclude.Model);
    }

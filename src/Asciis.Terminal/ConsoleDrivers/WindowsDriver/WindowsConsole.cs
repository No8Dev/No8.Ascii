...
internal class WindowsConsole
{
    ...
    private CursorVisibility? pendingCursorVisibility = null;
    ...
    {
        if (ScreenBuffer == IntPtr.Zero) ReadFromConsoleOutput(size, coords, ref window);
    ...
    public void ReadFromConsoleOutput(Size size, Coord coords, ref SmallRect window)
    {
        ...
        if (!initialCursorVisibility.HasValue && GetCursorVisibility(out var visibility))
            initialCursorVisibility = visibility;

        if (!SetConsoleActiveScreenBuffer(ScreenBuffer)) throw new Win32Exception(Marshal.GetLastWin32Error());
    ...
    public bool SetCursorPosition(Coord position) { return SetConsoleCursorPosition(ScreenBuffer, position); }

    public void SetInitialCursorVisibility()
    {
        if (initialCursorVisibility.HasValue == false && GetCursorVisibility(out var visibility))
            initialCursorVisibility = visibility;
    }

    public bool GetCursorVisibility(out CursorVisibility visibility)
    {
        if (!GetConsoleCursorInfo(ScreenBuffer, out var info))
        {
            var err = Marshal.GetLastWin32Error();
    ...
    {
        if (initialCursorVisibility.HasValue && pendingCursorVisibility.HasValue &&
    ...
    public void Cleanup()
    {
        if (initialCursorVisibility.HasValue) SetCursorVisibility(initialCursorVisibility.Value);
    ...
    internal Size SetConsoleWindow(short cols, short rows)
    {
        ...
        if (!SetConsoleWindowInfo(
            ScreenBuffer,
            true,
            ref winRect)) //throw new System.ComponentModel.Win32Exception (Marshal.GetLastWin32Error ());
    ...
    public uint ConsoleMode
    {
        get
        {
            GetConsoleMode(InputHandle, out var v);
            return v;
        }
    ...
    public uint InputEventCount
    {
        get
        {
            GetNumberOfConsoleInputEvents(InputHandle, out var v);
            return v;
        }
    }

...
internal class DirListView : View
{
    ...
    private FileDialog host;

    public DirListView(AsciiApplication application, FileDialog host)
    {
    ...
    private void Watcher_Error(object sender, ErrorEventArgs e) { Application.MainLoop.Invoke(() => Reload()); }
    ...
    public override void PositionCursor() { Move(0, selected - top); }
    ...
    private void OnSelectionChanged()
    {
        if (allowsMultipleSelection)
        {
            if (FilePaths.Count > 0)
                ...
                FileChanged?.Invoke(
                    infos[selected].Item2 && !canChooseDirectories ? "" : Path.GetFileName(infos[selected].Item1));
        }
        else
        {
    ...
    public bool GetValidFilesName(string files, out string result)
    {
        ...
        var valid = true;
        IReadOnlyList<string> filesList =
            new List<string>(files.Split(splitString.ToArray(), StringSplitOptions.None));
        var filesName = new List<string>();
        UnMarkAll();

        foreach (var file in filesList)
        {
            if (!allowsMultipleSelection && filesName.Count > 0) break;
            ...
            {
                if (canChooseDirectories && !canChooseFiles && !infos[idx].Item2) valid = false;
                if (allowsMultipleSelection && !infos[idx].Item3)
                    infos[idx] = (infos[idx].Item1, infos[idx].Item2, true);
                if (!allowsMultipleSelection) selected = idx;
        ...
        if (string.IsNullOrEmpty(result)) valid = false;
        return valid;
    }
    ...
    internal bool ExecuteSelection(bool navigateFolder = true)
    {
        ...
        if (isDir)
        {
            ...
            if (canChooseDirectories && !navigateFolder) return true;
        }
        else
        {
            OnSelectionChanged();
            ...
            else
            {
                ...
                if (infos[selected].Item2)
                {
                    if (canChooseDirectories)
                    {
                        ...
                        return sel == ".."
                            ? new List<string>()
                            : new List<string>() { MakePath(infos[selected].Item1) };
                    }
...
{
    private Button prompt, cancel;
    private Label nameFieldLabel, message, nameDirLabel;
    private TextField dirEntry, nameEntry;
    internal DirListView dirListView;
    private ComboBox cmbAllowedTypes;

    /// <summary>
    /// Initializes a new <see cref="FileDialog"/>.
    /// </summary>
    public FileDialog(AsciiApplication application)
        : this(
              application, string.Empty, string.Empty, string.Empty, string.Empty)
    { }
    ...
    public FileDialog(
        ...
        : this(application, title, prompt, string.Empty, nameFieldLabel, message, allowedTypes) { }
    ...
    /// <param name="message">The message.</param>
    /// <param name="allowedTypes">The allowed types.</param>
    public FileDialog(AsciiApplication application, string title, string prompt, string message, List<string> allowedTypes)
        : this(application, title, prompt, string.Empty, message, allowedTypes) { }
    ...
    public FileDialog(
        ...
        string message,
        List<string> allowedTypes = null)
        : base(application, title, 0, 0) //, Driver.Cols - 20, Driver.Rows - 5, null)
    {
        ...
        Add(this.message);
        var msgLines = TextFormatter.MaxLines(message, 60);
        ...
        dirListView = new DirListView(Application, this)
        {
            X = 1,
            Y = 3 + msgLines + 2,
            Width = Dim.Fill() - 1,
        ...
        {
            IsDefault = true,
            Enabled = nameEntry.Text.IsEmpty()
                              ? false
                              : true
        };
        this.prompt.Clicked += () =>
        {
            if (this is OpenDialog)
            {
                ...
                if (!dirListView.canChooseDirectories && !dirListView.ExecuteSelection(false)) return;
            }
            else if (this is SaveDialog)
            {
                var name = nameEntry.Text.ToString();
                if (FilePath.IsEmpty() || name.Split(',').Length > 1) return;
...
///   <see cref="MediaTypeNames.Application.Run(Func{Exception, bool})"/>. This will run the dialog modally,
...
public class SaveDialog : FileDialog
{
    ...
    /// </summary>
    public SaveDialog(AsciiApplication application)
        : this(application, string.Empty, string.Empty) { }
    ...
    /// <param name="message">The message.</param>
    /// <param name="allowedTypes">The allowed types.</param>
    public SaveDialog(AsciiApplication application, string title, string message, List<string> allowedTypes = null)
        : base(application, title, "Save", "Save as:", message, allowedTypes) { }
...
public class OpenDialog : FileDialog
{
    ...
    /// </summary>
    public OpenDialog(AsciiApplication application)
        : this(application, string.Empty, string.Empty) { }
    ...
    public OpenDialog(
        ...
        : base(
            application,
            title,
            openMode == OpenMode.File ? "Open" : openMode == OpenMode.Directory ? "Select folder" : "Select Mixed",
            "Open",
            message,
            allowedTypes)
    {
    ...
    public bool AllowsMultipleSelection
    {
        ...
        set
        {
            if (!value && openMode == OpenMode.Mixed) return;

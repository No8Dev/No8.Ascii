...
public class Rect
{
    ...
    public int Left => Math.Min(X, X + Width - 1);
    public int Top => Math.Min(Y, Y + Height - 1);
    ...
    public static implicit operator Rect(GridLayout.Zone zone) 
        => Create(zone.X, zone.Y, zone.Width, zone.Height);

...
public class NodeArrange
{
    ...
    public static void Calculate(INode node,
    ...
    private void CalculateLayout(INode node,
    ...
    /// </summary>
    private bool LayoutNode(INode node,
    ...
    private void LayoutImplementation(INode node,
                                       float availableWidth,
                                       float availableHeight,
                                       UIDirection containerUIDirection,
                                       MeasureMode widthMeasureMode,
                                       ...
                                       int generationCount)
    {
        ...
        var mainLengthOverflows = measureModeMainDim != MeasureMode.Undefined && totalOuterMainLength > availableInnerMainDim;
                    ...
                    else
                    {
                         ...
                         && MarginTrailingValue(element, crossAxis).Unit != Number.UoM.Auto)
                        {
                            ...
                            if (!IsLayoutDirectionInPlan(element, crossAxis, availableInnerCrossDim))
                            {
                                ...
                                ConstrainMaxSizeForMode(element,
                                                        mainAxis,
                                                        availableInnerMainDim,
                                                        availableInnerWidth,
                                ...
                                var elementWidthMeasureMode = elementWidth.IsUndefined() || !isMainAxisRow && crossAxisDoesNotGrow
                                                                ? MeasureMode.Undefined
                                                                : MeasureMode.Exactly;
                                var elementHeightMeasureMode = elementHeight.IsUndefined() || isMainAxisRow && crossAxisDoesNotGrow
                                                                 ? MeasureMode.Undefined
                                                                 : MeasureMode.Exactly;
                        ...
                        {
                            var remainingCrossDim = containerCrossAxis - DimWithMargin(element, crossAxis, availableInnerWidth);

                            if (MarginLeadingValue(element, crossAxis).Unit == Number.UoM.Auto && MarginTrailingValue(element, crossAxis).Unit == Number.UoM.Auto)
                                ...
                                case AlignmentCross.Center:
                                    {
                                        ...
                                        element.Layout.Position[Pos[(int)crossAxis]] = currentLead + (lineHeight - elementHeight) / 2;
                                ...
                                case AlignmentCross.Stretch:
                                    {
                                        ...
                                        {
                                            var elementWidth = isMainAxisRow
                                                                 ? element.Layout.MeasuredDimensions.Width + GetMarginForAxis(element, mainAxis, availableInnerWidth)
                                                                 : lineHeight;

                                            var elementHeight = !isMainAxisRow
                                                                  ? element.Layout.MeasuredDimensions.Height + GetMarginForAxis(element, crossAxis, availableInnerWidth)
                                                                  : lineHeight;
        ...
        SetLayoutMeasuredDimension(node,
                                   BoundAxis(node,
                                             LayoutDirection.Row,
            ...
            SetLayoutMeasuredDimension(node,
                                       FloatMax(
                                                FloatMin(
                                                         ...
                                                         BoundAxisWithinMinAndMax(node, crossAxis, totalLineCrossDim + paddingAndBorderAxisCross, crossAxisContainerSize)
                                                        ),
                                                paddingAndBorderAxisCross),
                                       Dim[(int)crossAxis]);
        }
        ...
        if (performLayout)
        {
            ...
            var needsMainTrailingPos = mainAxis == LayoutDirection.RowReverse || mainAxis == LayoutDirection.ColumnReverse;
            var needsCrossTrailingPos = crossAxis == LayoutDirection.RowReverse || crossAxis == LayoutDirection.ColumnReverse;
    ...
    private float ComputeMainLengthForElements(INode node,
                                               ...
                                               int generationCount)
    {
        ...
        var elements = new List<INode>(node.Elements);
    ...
    private void ComputeMainLengthForElement(INode node,
                                           INode element,
                                           float width,
                                           ...
                                           int generationCount)
    {
        ...
        else if (isMainAxisRow && isRowStyleDimDefined)
        {
            ...
            element.Layout.ComputedMainLength = Math.Max(element.Layout.GetResolvedDimension(Dimension.Width).Resolve(containerWidth),
                                                      paddingAndBorder);
        }
        else if (!isMainAxisRow && isColumnStyleDimDefined)
        {
            ...
            element.Layout.ComputedMainLength = Math.Max(element.Layout.GetResolvedDimension(Dimension.Height).Resolve(containerHeight),
                                                      paddingAndBorder);
        }
        else
        {
            ...
            {
                elementHeight = element.Layout.GetResolvedDimension(Dimension.Height).Resolve(containerHeight) + marginColumn;
    ...
    private void AbsoluteLayoutElement(INode container,
                                      INode element,
                                      float width,
                                      MeasureMode widthMode,
                                      float height,
                                      ...
                                      int generationCount)
    {
        ...
        else
        {
             ...
             && IsTrailingPosDefined(element, LayoutDirection.Row))
            {
                elementWidth = container.Layout.MeasuredDimensions.Width
                           - (GetLeadingBorder(container, LayoutDirection.Row) + GetTrailingBorder(container, LayoutDirection.Row))
                           - (GetLeadingPosition(element, LayoutDirection.Row, width) + GetTrailingPosition(element, LayoutDirection.Row, width));
        ...
        else
        {
             ...
             && IsTrailingPosDefined(element, LayoutDirection.Column))
            {
                elementHeight = container.Layout.MeasuredDimensions.Height
                            - (GetLeadingBorder(container, LayoutDirection.Column) + GetTrailingBorder(container, LayoutDirection.Column))
                            - (GetLeadingPosition(element, LayoutDirection.Column, height) + GetTrailingPosition(element, LayoutDirection.Column, height));
                elementHeight = BoundAxis(element,
                                        LayoutDirection.Column,
                                        elementHeight,
                                        height,
                                        width);
        ...
        if (elementWidth.IsUndefined() || elementHeight.IsUndefined())
        {
            ...
            LayoutNode(
                       ...
                       depth,
                       generationCount);
            elementWidth = element.Layout.MeasuredDimensions.Width + GetMarginForAxis(element, LayoutDirection.Row, width);
        ...
        else if (!IsLeadingPositionDefined(element, crossAxis) && AlignElement(container, element) == AlignmentCross.Center)
        {
            element.Layout.Position[Leading[(int)crossAxis]] =
                (container.Layout.MeasuredDimensions[Dim[(int)crossAxis]]
                - element.Layout.MeasuredDimensions[Dim[(int)crossAxis]])
              ...
              && (AlignElement(container, element) == AlignmentCross.End) ^ (container.Plan.LayoutWrap == LayoutWrap.WrapReverse))
        {
            element.Layout.Position[Leading[(int)crossAxis]] =
                container.Layout.MeasuredDimensions[Dim[(int)crossAxis]]
              - element.Layout.MeasuredDimensions[Dim[(int)crossAxis]];
    ...
    private void SetMeasuredDimensions_MeasureFunc(INode node,
                                                    float availableWidth,
                                                    float availableHeight,
                                                    MeasureMode widthMeasureMode,
                                                    MeasureMode heightMeasureMode,
                                                    ...
                                                    float containerHeight)
    {
        ...
        else
        {
            ...
            var measuredSize = node.MeasureNode?.Invoke(node,
                                                        innerWidth,
                                                             widthMeasureMode,
                                                             innerHeight,
                                                             heightMeasureMode)
    ...
    private void SetMeasuredDimensions_EmptyContainer(INode node,
                                                       float availableWidth,
                                                       float availableHeight,
                                                       MeasureMode widthMeasureMode,
                                                       MeasureMode heightMeasureMode,
    ...
    private bool SetMeasuredDimensions_FixedSize(INode node,
                                                  float availableWidth,
                                                  float availableHeight,
                                                  MeasureMode widthMeasureMode,
                                                  MeasureMode heightMeasureMode,
    ...
    private void ZeroOutLayoutRecursively(INode container)
    {
        Traverse(
                 container,
                 n => n.Layout = new Layout { Width = 0f, Height = 0f });
    }
    ...
    private float CalculateAvailableInnerDim(INode node,
    ...
    private CollectFlexItemsRowValues CalculateCollectFlexItemsRowValues(INode node,
                                                                          UIDirection containerUIDirection,
                                                                          ...
                                                                          int lineCount)
    {
        var flexAlgoRowMeasurement = new CollectFlexItemsRowValues { RelativeElements = new List<INode>(node.Elements.Count) };

        float sizeConsumedOnCurrentLineIncludingMinConstraint = 0;
        var mainAxis = ResolveLayoutDirection(node.Plan.LayoutDirection, ResolveUIDirection(node, containerUIDirection));
        var isLayoutWrap = node.Plan.LayoutWrap != LayoutWrap.NoWrap;

        // Add items to the current line until it's full or we run out of items.
        var endOfLineIndex = startOfLineIndex;
        for (; endOfLineIndex < node.Elements.Count; endOfLineIndex++)
        {
            var element = node.Elements[endOfLineIndex];
            ...
            var mainLengthWithMinAndMaxConstraints = BoundAxisWithinMinAndMax(element, mainAxis, element.Layout.ComputedMainLength, mainAxisContainerSize);
            ...
            sizeConsumedOnCurrentLineIncludingMinConstraint += mainLengthWithMinAndMaxConstraints + elementMarginMainAxis;
            flexAlgoRowMeasurement.SizeConsumedOnCurrentLine += mainLengthWithMinAndMaxConstraints + elementMarginMainAxis;
            ...
            if (IsNodeFlexible(element))
            {
                ...
                flexAlgoRowMeasurement.TotalFlexShrinkScaledFactors += -ResolveFlexShrink(element) * element.Layout.ComputedMainLength;
            }
    ...
    private void DistributeFreeSpace_FirstPass(CollectFlexItemsRowValues collectedFlexItemsValues,
                                                LayoutDirection mainAxis,
                                                float mainAxisContainerSize,
                                                float availableInnerMainDim,
                                                float availableInnerWidth)
    {
        ...
        foreach (var currentRelativeElement in collectedFlexItemsValues.RelativeElements)
        {
            var elementMainLength =
                BoundAxisWithinMinAndMax(currentRelativeElement,
                                         mainAxis,
            ...
            if (collectedFlexItemsValues.RemainingFreeSpace < 0)
            {
                ...
                if (flexShrinkScaledFactor.HasValue() && flexShrinkScaledFactor.IsNotZero())
                {
                    ...
                    boundMainSize = BoundAxis(currentRelativeElement,
                                              mainAxis,
                                              baseMainSize,
                                              availableInnerMainDim,
                                              availableInnerWidth);
    ...
    private float DistributeFreeSpace_SecondPass(CollectFlexItemsRowValues collectedFlexItemsValues,
                                                  INode node,
                                                  LayoutDirection mainAxis,
                                                  ...
                                                  int generationCount)
    {
        ...
        foreach (var currentRelativeElement in collectedFlexItemsValues.RelativeElements)
        {
            var elementMainLength = BoundAxisWithinMinAndMax(currentRelativeElement,
                                                          mainAxis,
                                                          currentRelativeElement.Layout.ComputedMainLength,
                                                          mainAxisContainerSize);
             ...
             && collectedFlexItemsValues.RemainingFreeSpace < 0)
            {
                ...
                if (flexShrinkScaledFactor.IsNotZero())
                {
                    ...
                    updatedMainSize = BoundAxis(currentRelativeElement,
                                                mainAxis,
                                                elementSize,
                                                availableInnerMainDim,
                                                availableInnerWidth);
            ...
            else
            {
                elementCrossSize = currentRelativeElement.Layout
                               ...
                               + marginCross;
                var isLoosePercentageMeasurement = currentRelativeElement.Layout.GetResolvedDimension(Dim[(int)crossAxis]).Unit == Number.UoM.Percent
                                                && measureModeCrossDim != MeasureMode.Exactly;
            ...
            ConstrainMaxSizeForMode(currentRelativeElement,
                                    mainAxis,
                                    availableInnerMainDim,
                                    availableInnerWidth,
                                    ref elementMainMeasureMode,
            ...
            var requiresStretchLayout = !IsLayoutDirectionInPlan(currentRelativeElement, crossAxis, availableInnerCrossDim)
                                     && AlignElement(node, currentRelativeElement) == AlignmentCross.Stretch
                                     && MarginLeadingValue(currentRelativeElement, crossAxis).Unit != Number.UoM.Auto
                                     && MarginTrailingValue(currentRelativeElement, crossAxis).Unit != Number.UoM.Auto;
    ...
    private void ResolveFlexibleLength(INode node,
    ...
    private void JustifyMainAxis(INode node,
                                  CollectFlexItemsRowValues collectedFlexItemsValues,
                                  int startOfLineIndex,
                                  LayoutDirection mainAxis,
                                  LayoutDirection crossAxis,
                                  ...
                                  bool performLayout)
    {
        ...
        if (measureModeMainDim == MeasureMode.AtMost && collectedFlexItemsValues.RemainingFreeSpace > 0)
        {
             ...
             && node.Plan.MinDimension(Dim[(int)mainAxis]).Resolve(mainAxisContainerSize).HasValue())
            {
                ...
                collectedFlexItemsValues.RemainingFreeSpace = FloatMax(0, minAvailableMainDim - occupiedSpaceByElements);
            }
            else
                collectedFlexItemsValues.RemainingFreeSpace = 0;
        }
            ...
            else
            {
                ...
                if (element.Plan.PositionType == PositionType.Relative)
                {
                    ...
                    else
                    {
                        ...
                        collectedFlexItemsValues.MainDim += betweenMainDim + DimWithMargin(element, mainAxis, availableInnerWidth);
...
//#endif
    ...
    private bool MeasureMode_SizeIsExactAndMatchesOldMeasuredSize(MeasureMode sizeMode,
                                                                   float size,
                                                                   float lastComputedSize) =>
        sizeMode == MeasureMode.Exactly && FloatsEqual(size, lastComputedSize);

    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private bool MeasureMode_OldSizeIsUnspecifiedAndStillFits(MeasureMode sizeMode,
                                                               float size,
    ...
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    private bool MeasureMode_NewMeasureSizeIsStricterAndStillValid(MeasureMode sizeMode,
                                                                    float size,
                                                                    MeasureMode lastSizeMode,
                                                                    float lastSize,
                                                                    float lastComputedSize) =>
    ...
    private bool CanUseCachedMeasurement(MeasureMode widthMode,
                                          float width,
                                          MeasureMode heightMode,
                                          float height,
                                          ...
                                          float marginColumn)
    {
        ...
        var widthIsCompatible = hasSameWidthSpec
                             || MeasureMode_SizeIsExactAndMatchesOldMeasuredSize(widthMode,
                                                                                 width - marginRow,
                                                                                 lastComputedWidth)
                             || MeasureMode_OldSizeIsUnspecifiedAndStillFits(widthMode,
                                                                             width - marginRow,
                                                                             lastWidthMode,
                                                                             lastComputedWidth)
                             || MeasureMode_NewMeasureSizeIsStricterAndStillValid(widthMode,
                                                                                  width - marginRow,
                                                                                  lastWidthMode,
                                                                                  lastWidth,
                                                                                  lastComputedWidth);
        ...
        var heightIsCompatible = hasSameHeightSpec
                              || MeasureMode_SizeIsExactAndMatchesOldMeasuredSize(heightMode,
                                                                                  height - marginColumn,
                                                                                  lastComputedHeight)
                              || MeasureMode_OldSizeIsUnspecifiedAndStillFits(heightMode,
                                                                              height - marginColumn,
                                                                              lastHeightMode,
                                                                              lastComputedHeight)
                              || MeasureMode_NewMeasureSizeIsStricterAndStillValid(heightMode,
                                                                                   height - marginColumn,
                                                                                   lastHeightMode,
                                                                                   lastHeight,
                                                                                   lastComputedHeight);
    ...
    private void RoundToPixelGrid(INode node, float pointScaleFactor, float absoluteLeft, float absoluteTop)
    {
        ...
        SetLayoutPosition(node,
                          Number.RoundValueToPixelGrid(nodeLeft, pointScaleFactor, false, textRounding),
                          Side.Left);

        SetLayoutPosition(node,
                          Number.RoundValueToPixelGrid(nodeTop, pointScaleFactor, false, textRounding),
                          Side.Top);
        ...
        node.Layout.Width =
            Number.RoundValueToPixelGrid(absoluteNodeRight,
                                         pointScaleFactor,
                                         textRounding && hasFractionalWidth,
                                         textRounding && !hasFractionalWidth)
          - Number.RoundValueToPixelGrid(absoluteNodeLeft,
                                         pointScaleFactor,
                                         false,
                                         textRounding);

        node.Layout.Height =
            Number.RoundValueToPixelGrid(absoluteNodeBottom,
                                         pointScaleFactor,
                                         textRounding && hasFractionalHeight,
                                         textRounding && !hasFractionalHeight)
          - Number.RoundValueToPixelGrid(absoluteNodeTop,
                                         pointScaleFactor,
                                         false,
                                         textRounding);
        ...
        if (node.Container != null)
        {
            node.Layout.DisplayLocation =
                new VecF(node.Container.Layout.DisplayLocation.X + node.Layout.Left,
                         node.Container.Layout.DisplayLocation.Y + node.Layout.Top);
        }
        else
        {
        ...
        for (var i = 0; i < elementCount; i++)
        {
            RoundToPixelGrid(node.Elements[i],
                             pointScaleFactor,
                             absoluteNodeLeft,
                             absoluteNodeTop);
        }
    ...
    {
        if (!node.Plan.MaxDimension(Dimension.Width).IsUndefined && node.Plan.MaxDimension(Dimension.Width) == node.Plan.MinDimension(Dimension.Width))
        ...
        if (!node.Plan.MaxDimension(Dimension.Height).IsUndefined && node.Plan.MaxDimension(Dimension.Height) == node.Plan.MinDimension(Dimension.Height))
    ...
    private bool IsLayoutDirectionInPlan(INode node, LayoutDirection axis, float containerSize)
    {
        ...
        return !(number.Unit == Number.UoM.Auto
               ...
               || number.Unit == Number.UoM.Percent && !number.IsUndefined && (number.Value < 0.0f || containerSize.IsUndefined()));
    }
                             ...
                             float containerWidth)
    {
        ...
        var uiDirectionRespectingRoot = node.Container != null
                                            ? uiDirection
                                            : UIDirection.LTR;
        ...
        node.Layout.Position[Leading[(int)mainAxis]] = GetLeadingMargin(node, mainAxis, containerWidth) + relativePositionMain;
        node.Layout.Position[Trailing[(int)mainAxis]] = GetTrailingMargin(node, mainAxis, containerWidth) + relativePositionMain;
        node.Layout.Position[Leading[(int)crossAxis]] = GetLeadingMargin(node, crossAxis, containerWidth) + relativePositionCross;
        node.Layout.Position[Trailing[(int)crossAxis]] = GetTrailingMargin(node, crossAxis, containerWidth) + relativePositionCross;
    }
    ...
    public void ConstrainMaxSizeForMode(INode node,
                                         LayoutDirection axis,
    ...
    public void SetLayoutMargin(INode node, float margin, Side edge) =>
        node.Layout.Margin[edge] = margin;

    public void SetLayoutBorder(INode node, float border, Side edge) =>
        node.Layout.Border[edge] = border;

    public void SetLayoutPadding(INode node, float padding, Side edge) =>
        node.Layout.Padding[edge] = padding;

    public void SetLayoutPosition(INode node, float position, Side side) =>
        node.Layout.Position[side] = position;

    //*********************************************************

    public static bool IsLayoutTreeEqualToNode(INode node, INode other)
    {
        if (node.Elements.Count != other.Elements.Count) return false;

        if (node.Layout != other.Layout) return false;

        if (node.Elements.Count == 0) return true;

        for (var i = 0; i < node.Elements.Count; ++i)
        {
            if (!IsLayoutTreeEqualToNode(node.Elements[i], other.Elements[i]))
                return false;
        }

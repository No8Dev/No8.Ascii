...
public delegate float BaselineFunc(INode node, float width, float height);
...
public class LayoutPlan
{
    ...
    public Sides Margin
    {
        ...
        {
            if (value == null)
                value = new Sides(Number.Undefined);
            if (value != _margin)
            {
                if (_margin != null)
    ...
    public Number Dimension(Dimension dim)
    {
        switch (dim)
        {
            case NodeLayout.Dimension.Width:
                return Width;
            case NodeLayout.Dimension.Height:
                return Height;
            default:
    ...
    */
    ...
    public static bool operator ==(LayoutPlan left, LayoutPlan right) =>
        Equals(left, right);

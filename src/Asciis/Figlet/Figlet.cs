...
public partial class Figlet
{
    ...
    {
        var figlet = new Figlet(fontName, smushMode: smushMode);
        figlet.Add(str);
    ...
    /// </summary>
    private static readonly char[] DutchChar =
        { (char)196, (char)214, (char)220, (char)228, (char)246, (char)252, (char)223 };
    ...
    private int OutLineLength => _outLines[0].Length;

    private FigletChar MissingFigletChar => _missingFiglet ??= new('\0', _charheight);
    ...
    private void ReadFontChar(string[] lines, ref int index, char ch)
    {
        ...
        for (var row = 0; row < _charheight; row++)
        {
    ...
    {
        var resourceName = "No8.Ascii.Figlet.fonts." + filename;
    ...
    private void ReadFont(string fontFilename)
    {
        ...
        while (index < lines.Length)
        {
            ...
            if (int.TryParse(xtraLine.TrimStart(), out var theCh))
    ...
    private int SmushAmt()
    {
        ...
        for (var row = 0; row < _charheight; row++)
        {
            ...
            var amt = charbd + lineLength - 1 - linebd;
    ...
    public string Render(StringBuilder? sb = null)
    {
        ...
        for (var i = 0; i < _charheight; i++)
            sb.AppendWithCRLF(FormatString(_outLines[i].ToString()));
        ClearLines();

...
public class ArgsParser
{
    ...
    internal string Help(ArgsCommandMeta meta)
    {
        ...
        sb.AppendWithCRLF($"{meta.Name}  -  {meta.CommandAttr.Description}");
        sb.AppendCRLF();
        sb.AppendWithCRLF($"Usage: {names} [options] [arguments]");
    ...
    public string Help()
    {
        ...
        sb.AppendWithCRLF("Usage: [command] [command-options] [arguments]");
        sb.AppendCRLF();
        sb.AppendWithCRLF("Commands");
        foreach (var command in _commands)
        {
            var cmdName = string.Join('|', command.CommandAttr.Names);
            var flag    = (command == _defaultCommand) ? "*" : " ";

            sb.AppendWithCRLF($"  {flag}{cmdName.PadRight(biggest)}  {(command.CommandAttr.Description ?? "")}");
        }

...
public record Number(float Value, Number.UoM Unit)
{
    ...
    public static float RoundValue(float value, float pointScaleFactor, bool forceCeil, bool forceFloor)
    {
        ...
        if (fractal < 0)
        {
            ...
            //
            // Regarding `FloatMod`. For fractional negative numbers, `FloatMod` returns a
            // negative number. For example, `FloatMod(-2.2) = -0.2`. However, we want
            // `fractal` to be the number such that subtracting it from `value` will
            // give us `floor(value)`. In the case of negative numbers, adding 1 to
            // `FloatMod(value)` gives us this. Let's continue the example from above:
            //   - fractal = FloatMod(-2.2) = -0.2

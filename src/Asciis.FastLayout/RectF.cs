...
public class RectF
{
    public static readonly RectF Empty = new(VecF.Zero, VecF.Zero);
    ...
    public VecF TopLeft     => new(Left, Top);
    public VecF TopRight    => new(Right, Top);
    public VecF BottomLeft  => new(Left, Bottom);
    public VecF BottomRight => new(Right, Bottom);

    public VecF Center => new((Left + Right) / 2f, (Top + Bottom) / 2f);
    ...
    {
        return new RectF(X - distance, Y - distance, Width + (distance * 2),
            Height + (distance * 2));
    }

    public RectF Offset(float x, float y) => new(this.X + x, this.Y + y, Width, Height);
    ...
    public static implicit operator Rect(RectF rectF) => new((int)rectF.X, (int)rectF.Y, (int)rectF.Width, (int)rectF.Height);
    public static implicit operator RectF(Rect rect) => new(rect.X, rect.Y, rect.Width, rect.Height);
}
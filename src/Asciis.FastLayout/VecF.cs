...
public class VecF
{
    public static readonly VecF Zero    = new(0, 0);
    public static readonly VecF Unknown = new(float.MinValue, float.MinValue);
    ...
    public VecF Abs() => new(Math.Abs(X), Math.Abs(Y));

    public VecF Offset(float  x, float y) => new(X + x, Y + y);
    public VecF OffsetX(float x) => new(X + x, Y);
    public VecF OffsetY(float y) => new(X, Y + y);
